...
| `source-repo` | 외부 앱 레포 바인딩(`ukyi-app/<app>`) — poll-ghcr가 이 파일 있는 앱만 update-image 폴링(`tools/poll-ghcr.mjs`; 누락=폴링 밖, fail-closed) |

생성/변이는 owner가 homelab에서 `dispatch-mutation`(create-app/update-secrets/…)으로만. 직접 만들지 않는다.
