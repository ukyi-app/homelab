...
| `tools/` | 앱 플랫폼 DX **Node CLI** (`create-app`/`onboard-app`/`activate-app`·`audit-orphans` 등 — `dispatch-mutation`·`bump-poll`이 호출) + 단위 테스트(`tools/tests/`) |
...
- **생성/파괴 변이:** owner가 homelab에서 `dispatch-mutation`(workflow_dispatch) 실행 —
