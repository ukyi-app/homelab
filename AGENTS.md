...
| `tools/` | 앱 플랫폼 DX **Node CLI** (`create-app`/`onboard-app`/`activate-app`·`audit-orphans` 등 — 변이 디스패처·`bump-poll`이 호출) + 단위 테스트(`tools/tests/`) |
...
- **생성 변이:** owner가 homelab에서 액션별 디스패처(workflow_dispatch) 실행 —
  ...
  validator(`tools/validate-mutation.mjs`)가 계약표 강제. 전역 직렬화: `concurrency: homelab-mutation` + `queue: max`.
