...
검사 데이터와 파일 열거는 신규·삭제까지 반영한 후보 manifest에서 읽는다. 원장 **행**은 고정
파서/helper로 후보 `docs/memory-ledger.md`에서 읽고, **예산 상한**은 coordinator가 고정한
신뢰된 검사 기준 revision의 원장 활성 `ledger:meta`의 `LIMIT_BUDGET_MIB`에서 읽는다. 기준 revision·원장 blob hash·
추출 budget을 신뢰 입력 manifest에 남긴다. 기준 상한의 누락·중복·잘못된 양의 정수는 검증 불가이며
후보 값으로 대체하지 않는다. 현재 10240이라는 숫자를 코드에 별도 하드코딩하지 않는다.

`ledger-to-json.ts`의 후보 JSON에는 후보 budget이 들어 있으므로 그 JSON을 그대로 고정 정책에
넘기지 않는다. 검증한 후보 rows와 신뢰된 기준 budget으로 새 입력을 구성해 고정 `ledger.rego`에
전달한다. 후보 상한의 변경·삭제 제안은 허용하고 변경 사실/파싱 가능 여부를 보고한다. 유효한
후보 상한 기준의 결과는 별도로 기록하며 고정 검사의 결과나 신뢰 입력을 덮지 않는다. 결과에는
기준/후보 상한, 행 합계, 각 평가와 메타데이터 변경 여부를 남긴다.
후보 메타데이터가 없거나 기형이어도 고정 `parseLedgerRows`와 마커 수/행 수 대조로 행을 별도
검증한다. 행 파싱 실패를 일부 행만의 통과로 바꾸지 않고 후보 메타데이터 삭제/기형도 결과에 표시한다.
...
기준 원장(10240 MiB)을 유지한 채 후보가 초과 행과 `LIMIT_BUDGET_MIB=50000`을 함께 넣어도
기존 기준 평가는 실패해야 한다. 메타데이터만 변경한 후보는 행의 기존 기준 평가와 상한 변경
표시가 함께 나와야 한다. 기준 상한 누락/중복/기형에서는 후보의 정상 상한으로 fallback하지
않아야 한다. 이 대조군은 후보 파서/helper/정책을 전혀 바꾸지 않고도 수행한다.
후보 상한 삭제/기형에서도 행이 기존 상한을 초과하면 실패하고, 행의 마커/파싱 불일치는 거부해야 한다.
