...
수집 단계도 2분·512 MiB·출력 1 MiB 상한을 적용한다. Codex는 2 GiB·출력 합계 8 MiB이며
단계 내부 재시도는 시간/출력 예산에 합산한다. 제한은 출력 적재 후가 아닌 스트리밍 중에 적용한다.
01번의 외부 감독기와 실행 cgroup 종료 계약을 사용하고 worker 내부 timer만으로 대체하지 않는다.
...
끝나지 않는 수집 query·출력 폭주·OOM·TERM을 무시하는 자식이 있어도 상한과 전체 자식 종료를
집행하는지 확인한다. 초과는 부분 결과와 실패로 남기고 완전한 증거나 성공으로 표시하지 않는다.
