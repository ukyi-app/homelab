...
예를 들어 원장은 기준의 파서/helper와 `policy/ledger.rego`로 **후보의**
`docs/memory-ledger.md`를 검사한다. 파일 열거도 기준 git index가 아닌 후보 manifest를 사용한다.
...
| 검증 경계 | 기준은 정상·후보만 위반인 대조군을 검출. 후보 helper/정책 약화로 고정 검사를 우회하지 못함. 실제 fork Draft PR에서 gate/plan 생략 동작 확인 |
