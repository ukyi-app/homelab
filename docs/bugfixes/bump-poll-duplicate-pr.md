...
red-baseline: db17b9c3acd46740ce131d2d07c47124ddf81fee
bugfix-lock: red
first-increment: [B-1]
increments: [B-1]
spike-1:
...
- **F-0 (권장, 이 픽스의 한계)**: `bump-poll/**` ref를 **writer App 전용으로 예약하는 GitHub ruleset**
  ...
  ★ **R-46 잔여 TOCTOU도 이 룰셋만이 닫는다**: 초기 스캔~force-push(또는 재확인~push) 사이에 남이 이 결정적
  head로 다른 base PR을 열면 lease(OID만 본다)는 여전히 성공한다 — **git ref lease는 동시 PR 생성을 원리적으로
  막을 수 없다**(리뷰어 확인). 도구는 force-push 직전 재확인(③-b2)으로 창을 마이크로초로 좁혔을 뿐이고,
  환원 불가능한 잔여는 `bump-poll/**`를 writer App에 예약해 **남이 그 네임스페이스에 PR/ref를 못 만들게** 하는
  이 룰셋만이 닫는다.
