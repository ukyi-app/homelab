...
| `findings="$(awk … || true)"` — `|| true`가 awk fatal rc까지 삼켜 검출기가 죽어도 "0곳 OK" rc=0을 낸다(가드 본체 fail-open). 처방 세 겹(awk rc 포착 + 인자 사전 검증 + READFILES 대조)은 detect_run 커널이 소유하고 콜사이트는 awk 본문만 소유한다 | gate | `scripts/lib/guard.sh`, `tests/gates/test_guard-sh.bats`, `scripts/check-host-ports.sh`, `scripts/check-locale-collation.sh`, `scripts/check-bats-style.sh`, `tests/gates/test_host-ports.bats`, `tests/gates/test_locale-collation.bats`, `tests/gates/test_bats-style.bats` |
