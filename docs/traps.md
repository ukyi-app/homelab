...
| push 주기 > instant 룩백(기본 5m) → 룰 시리즈 구멍 → `for:` 영구 리셋 = 무발화(fail-open) | gate | `tests/gates/vmalert-drift-firing-e2e.sh`, `tests/gates/vmalert-bulkssd-firing-e2e.sh`, `tests/gates/test_digest-exporter.bats`, `tools/check-alert-rules.ts`(모드 C: 레포 전역 정적 lint + push 생산자 완전성 가드), `tests/test_alert_rules.bats` |
