...
- **where**: `gate`=ci.yaml job `gate`가 수집 · `verify`=verify.yml(pre-commit/sops/ledger) · `iac`=iac/tf-reconcile · `local`=make/pre-commit 로컬.
...
| vmalert configCheckInterval 없으면 룰 자동 reload 안 함 | gate | `tools/test/test_vmalert-config.bats` |
| Alertmanager telegram: 자동 HTML-escape(이중 escape 금지) + 계약 | gate | `tools/test/alertmanager-render-e2e.sh`, `tools/test/test_telegram-notify.bats`, `tools/test/test_telegram-alert-korean.bats`, `tools/test/test_telegram-callsites.bats` |
| GitHub Actions client_payload는 비신뢰 입력(env 경유+regex) | gate | `tools/test/test_dispatcher.bats`, `tools/test/test_validate-mutation.bats` |
| concurrency queue:max ↔ cancel-in-progress 병용 불가(dispatcher 직렬화) | gate | `tools/test/test_dispatcher.bats` |
| 워크플로 YAML colon-in-unquoted-name 문법 깨짐 | gate | `tools/test/test_workflow-yaml.bats` |
...
| enc.yaml 평문 직접 수정 금지(SOPS MAC) | gate+verify | `scripts/sops-guard.sh`, `.claude/hooks/manifest-guard.sh`, `tools/test/test_manifest-guard.bats`, `tools/test/test_verify-secrets.bats` |
| SOPS 왕복(암호화 후 복호 동일) | local | `tests/test_sops-roundtrip.bats` |
| `.claude/` 선택적 un-ignore(하네스 추적/런타임 무시) | gate | `tools/test/test_claude-harness-tracked.bats` |
| make ci ↔ ci.yaml gate 8스텝 패리티 | gate | `tools/test/test_make-ci-parity.bats` |
