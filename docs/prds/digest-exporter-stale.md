...
- **라이브 증거**(hermetic 하네스가 도달할 수 없는 층): 랜딩 후 verification에서 **실제 첫 하트비트
  도착 지연**을 기록한다(ArgoCD 싱크 시각 → vmsingle의 첫 `digest_exporter_last_success_timestamp`
  샘플). 강제 상한(780s) 대비 실측이 어디에 있는지가 이 계약의 최종 확인이다.
