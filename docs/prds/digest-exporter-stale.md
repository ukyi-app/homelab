...
| 최악 지연 요소 | 상한 |
|---|---|
| vmalert 룰 로드(`configCheckInterval`) | 30s |
| 다음 `*/10` 크론 경계까지 | 10m |
| 파드 스케줄 + skopeo 이미지 기동(digest 핀·노드 캐시) | ~1m |
| skopeo inspect × 앱 수 + curl push | ~30s |
| **합계(최악 첫 하트비트)** | **≈ 12m** |

→ **`T = 900s` · `for: 15m`** 채택. `for: 15m > 12m`이므로 첫 하트비트가 도착하면 pending이 리셋되어
**거짓 페이지가 구조적으로 불가능**하다. 동시에 실제 사망 시에는 `time()-hb > 900`이 T0+15분에 참이
되고 `for: 15m` 뒤 **T0+30분에 발화** — owner 결정 ③의 값과 **동일**하다.
...
- **테스트로 못박는다**: 발화 e2e에 **부트스트랩 레그(L7)** 를 추가한다 — 하트비트가 평가 시작 시점에
  없고 +10분에 처음 도착하는 시계열 → `DigestExporterStale` **무발화**. 이 레그가 미래에 누군가
  `for:`를 줄이거나 T/for 분할을 뒤집는 것을 막는다.
...
- **레그 구성(7)** — plan 게이트 P-3 반영(L5·L6 추가, L2 강화) + P-2 최종 해법 잠금(L7):
  ...
  | L7 | **부트스트랩**: 평가 시작 시점에 하트비트 없음 → **+10분에 첫 샘플** 도착 | `DigestExporterStale` **무발화** — 최초 배포 거짓 페이지가 구조적으로 불가능함을 증명한다(`for: 15m` > 최악 첫 하트비트 ≈12분). 누군가 `for:`를 줄이거나 T/for 분할을 뒤집으면 여기서 죽는다 |
...
- **비용**: 레그마다 vmsingle 컨테이너를 새로 띄운다(형제 실측: drift ≈69~95s, bulkssd 4레그 ≈142s).
  ...
  비하면 수용 가능하다고 판단한다.
- **preflight 산술**: `for`(15m)가 eval(30s)의 정수배 ✓ · rollup 윈도 W(2h) ≥ push 주기(600s) ✓ ·
  룩백(300s) < push 주기(구멍 전제) ✓ · **`for` > 최악 첫 하트비트 지연**(부트스트랩 불변식, L7의 계약).
...
**해소(최종)**: **배포를 쪼개는 대신 룰을 부트스트랩 안전하게 만든다 — 단일 랜딩.** owner 결정 ③이
고정한 것은 **발화 시각 T0+30분**이고 `T`/`for:` 분할은 자유도다. `for:`를 최악의 첫 하트비트 지연
(≈12분: 룰 로드 30s + 크론 경계 10m + 파드/skopeo ~1.5m)보다 크게 잡으면 첫 하트비트가 pending을
리셋해 **거짓 페이지가 구조적으로 불가능**해진다 → **`T = 900s` · `for: 15m`**(발화 시각 T0+30분과
누락 내성 30분 = 3주기는 **불변**). 앞서 거부한 "`for:` 유예 인플레"와 다르다 — 그건 T를 유지한 채
발화를 T0+45분으로 **미루는** 안이었고, 이건 총예산을 보존한 채 분할만 바꾼다. 발화 e2e에 **부트스트랩
레그(L7)** 를 추가해 이 성질을 테스트로 못박았다. owner가 하드룰 4의 (b) 수동 라운드를 승인했고
(2026-07-13, "특이사항 없으면 권장안대로 진행"), 그 승인 하에 r5로 재검증한다.
