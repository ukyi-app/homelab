...
- **해법 = 순서**(유예가 아니다). 두 슬라이스를 **분리 배포**한다:
  1. **슬라이스 1(skeleton)**: exporter 메트릭 3개 + 레지스트리 등재 + producer 행위 테스트.
     → 배포 후 **첫 하트비트가 vmsingle에 적재됐음을 라이브로 확인**한다.
  2. **슬라이스 2**: 알림 룰 2건 + 발화 e2e 게이트 + 함정 원장 tie.
     → 룰이 로드되는 시점에 하트비트는 이미 최근 값이므로 `absent`도 `time()-hb > 1200`도 거짓이다.
- **롤백은 역순**: 룰을 먼저 제거하고(알림 침묵) 그다음 exporter 메트릭을 되돌린다. 역순을 어기면
  메트릭이 사라진 상태로 룰만 남아 즉시 페이징한다.
- **`for:` 유예 인플레는 거부한다.** 초기화 유예(예: `for: 25m`)로 이 경주를 덮으면 발화가 T0+45분으로
  밀려 owner가 15분으로 좁힌 실명 구간이 그대로 원복된다(위 산술표). 순서로 푸는 쪽이 산술을 보존한다.
- 이 순서가 곧 **walking skeleton의 경계**다 — 슬라이스 1이 skeleton(가장 위험한 통합: 스크립트가
  실제로 옳게 세고 push하는가)이고, 슬라이스 2는 그 위에 얹힌다.
