...
| CronJob `jobTemplate.spec.activeDeadlineSeconds` | **120s** | 어떤 잡도 2분 넘게 못 산다(정상 실행은 ~10~20초). 행 잡이 슬롯을 무한 점유하는 경로를 차단 |
...
+ 노드 캐시) + `activeDeadlineSeconds`(120s) = **780s = 13분**. `Replace` 덕에 이 상한은 **랜딩 시점에
구 Job이 돌고 있든 없든 성립**한다(구 Job은 다음 크론 경계에 교체된다).
...
- **preflight 산술(전부 매니페스트에서 파생 — 하드코딩 금지)**: `for`(15m)가 eval(30s)의 정수배 ✓ ·
  ...
  **부트스트랩 불변식 3개**(digest-exporter.yaml에서 `yq`로 읽어 강제, 위반 = **exit 2 HARNESS FAULT**):
  ...
  셋 중 하나라도 깨지면 최초 배포 거짓 페이지가 되살아나므로 게이트가 먼저 죽는다.
- **정적 gate 단언**(`tests/gates/test_digest-exporter.bats`, e2e와 독립): CronJob이
  ...
  `--command-timeout`·curl `--max-time`을 쓴다. 누가 되돌리면 required gate가 red가 된다.
- **랜딩 전이 증거**(hermetic 하네스가 도달할 수 없는 층 — 실물 핀된 k3s에서 관측):
  verification에 다음을 기록한다. ①ArgoCD 싱크 **직전** `kubectl get jobs -n observability`(레거시 Job
  활성 여부 — 활성이었다면 그것이 곧 `Replace` 전이의 실물 관측이고, 비활성이었다면 전이 경로 자체가
  발생하지 않았음을 명시) ②싱크 후 후속 Job의 UID·생성 시각 ③**첫 하트비트 실측 지연 vs 강제 상한
  (780s)**. 이것이 이 계약의 최종 확인이다.
...
실측 지연 vs 780s 상한) — 합성 하네스가 아니라 핀된 실제 클러스터에서의 관측.
...
뜬다; 잡은 멱등이라 중도 종료 손실 0) + `activeDeadlineSeconds: 120` + skopeo `--command-timeout=30s`
