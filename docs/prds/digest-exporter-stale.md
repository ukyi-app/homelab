...
| skopeo `--command-timeout` | **10s** (env `SKOPEO_TIMEOUT` 기본값) | 앱당 GHCR 조회 상한(네트워크 블랙홀 대비). **argv 순서를 계약으로 고정**: `skopeo --command-timeout="$SKOPEO_TIMEOUT" inspect --authfile … "docker://$REF"`(글로벌 옵션을 서브커맨드 앞에). 정적 게이트는 존재가 아니라 **순서**를 단언한다. ⚠️ **정정(I-1 구현 중 실측 2026-07-13)**: plan 게이트 r9는 "`inspect` 뒤에 두면 무효"라고 주장했으나 **거짓**이다 — 핀된 v1.22.2에서 `--command-timeout`은 **cobra persistent flag**라 서브커맨드가 상속하며 뒤에 둬도 상한이 걸린다(실측: `=3s`→3초, `=9s`→9초 종료). 앞 배치를 그대로 계약으로 유지하는 이유는 ⓐ 스모크 게이트가 **실제로 증명한** 배치가 그것이고 ⓑ 뒤 배치의 수용은 cobra 상속이라는 구현 세부라 CLI 재구성 한 번에 사라질 수 있기 때문이다(계약은 유지, 근거만 교체) |
...
- **랜딩 후 라이브 체크포인트**(hermetic 하네스가 도달 못 하는 층 — 실물 핀된 k3s에서 관측).
  ...
  | ArgoCD 싱크 후 (**필수**) | `kubectl rollout restart deploy/alertmanager -n observability` → `kubectl rollout status deploy/alertmanager -n observability` | **신규 알림의 제목 매핑이 실행 중 파드에 도달했는지.** `alertmanager-config` ConfigMap은 initContainer(`render-config`)가 **파드 기동 시에만** emptyDir로 렌더한다 — checksum 어노테이션도 reloader도 없어 ArgoCD가 ConfigMap을 적용해도 **기존 파드는 옛 템플릿을 계속 쓴다**(함정 원장: `docs/traps-detail.md`「ConfigMap 변경 파드 자동 재시작 없음」). 재기동 없이 발화하면 새 알림이 **제목 없이/기본 제목으로** 나간다 → 근본 해결은 F-8 |
  | 재기동 후 | `kubectl exec deploy/alertmanager -n observability -c alertmanager -- cat /etc/alertmanager/alertmanager.yml \| grep -c DigestExporter` (렌더된 설정은 initContainer가 emptyDir `config-out`에 쓴 `/etc/alertmanager/alertmanager.yml`이다 — `grep`은 로컬에서 돌린다) | 신규 알림 2건(`DigestExporterStale`·`DigestExporterScrapeIncomplete`)의 제목 매핑이 **실행 중 파드**에 반영됐다는 증거(기대: ≥2) |
...
| ID | 항목 | 근거 |
|---|---|---|
| F-3 | `scripts/backup-files-data.sh` fail-loud화(df 실패 시 하트비트 미발행) | 이전 세션 릴리스 게이트 발굴 |
| F-4 | `make ci` ↔ ci.yaml 패리티 — `gate`의 `tests/gates/*.sh` 중 로컬에서 안 도는 것들(이번에 2개 추가) | Out of Scope 참조 |
| F-5 | drift 하네스를 공유 lib(`tests/gates/lib/vmalert-e2e.sh`)으로 이관 | gated-refactor(행위 보존) |
| **F-6** | `tests/gates/vmalert-bulkssd-firing-e2e.sh`의 잠복 bash 함정 — `fault` 메시지 안에서 한글이 변수명에 붙어(`$VISIBLE_EVALS회`) `set -u`가 "unbound variable"로 죽는다. HARNESS FAULT 경로에서만 발현해 현재는 휴면 | I-1 구현 중 발견(같은 함정을 신규 하네스에서 회피: `${ALERT}` 형태로 중괄호 사용) |
| **F-7** | `DEB_POD_START_BUDGET_S=60`(`tests/gates/lib/digest-exporter-budget.sh`)은 매니페스트 파생이 아닌 **가정**이다 — k8s에 잡 startup 상한 수단이 없어 N_MAX·첫-하트비트 상한이 관측 기반 가정 위에 선다. **강제 가능한 제약에서 파생**(startup SLA 도입, 또는 ADS/N_MAX를 pull 실측에서 재산정)으로 승격 | 릴리스 게이트 R-2(defer 판정: 가정이 깨져도 Job이 ADS에 걸려 죽고 하트비트 미발행 → Stale 발화 = **fail-closed**. 현재 N=2 여유 120 < 180) |
| **F-8** | Alertmanager ConfigMap 리로더/checksum 도입 — `alertmanager-config`는 initContainer가 **파드 기동 시에만** 렌더해서 ArgoCD 적용만으로는 실행 중 파드에 도달하지 못한다(현재는 랜딩 절차의 **수동 `rollout restart`**로 메운다). pod template에 config checksum 어노테이션을 넣어 변경 시 자동 롤아웃되게 한다 | 릴리스 게이트 R-4(랜딩 후 라이브 체크포인트에 수동 스텝 추가로 임시 대응) |

## Review Decision Log

### Landing — PR #355 머지(`bbc9bde`) · 라이브 검증 통과 (2026-07-13)

라이브 기록: `docs/reviews/digest-exporter-stale/live-verification.md`

- **첫 하트비트 실측 지연 115초**(ArgoCD 싱크 09:18:09 → 적재 09:20:04) — 강제 상한 **840초의 13.7%**.
- `DigestExporter*` 알림 **pending·firing 0** → **최초 배포 거짓 페이지 없음**(e2e L7이 hermetic하게
  증명한 성질의 라이브 확인).
- 카운트 `configured=2 / scraped=2`, vmalert에 신규 룰 2건 **LOADED**.
- **R-4가 라이브에서 실증**: alertmanager 재기동 **전** 매핑 0건 → 재기동 **후** 2건(ConfigMap→emptyDir
  경로에 reloader 부재). 항구 해결은 F-8.
- 싱크 시점 활성 레거시 Job **없음** → `Replace` 전이 경로는 이번 랜딩에서 발생하지 않았다(r7 잔여
  리스크는 그대로 유지).
- 별건: `ImageDigestDrift{app="page"}`가 firing — 이 기능과 무관하며, 살려낸 알림이 **실제 드리프트를
  정상 탐지** 중이다(별도 확인 필요).

### Codex Release Review — r1: needs-attention → 3건 Accept · 1건 Defer (owner 2026-07-13)

아티팩트: `docs/reviews/digest-exporter-stale/release-r1.json`(reviewedSha `a026629`).

| ID | 심각도 | 발견 | 결정 | 반영 |
|---|---|---|---|---|
| R-1 | high | `Heartbeat is not an atomic commit marker for the streamed batch` — 하트비트가 카운트 **앞**에 놓이고 `/api/v1/import/prometheus`는 **스트리밍**이라, 요청이 절단되면 **접두부(하트비트)만 적재**되고 카운트는 안 들어갈 수 있다 → 신선한 하트비트 + 낡은 카운트 → **두 알림 모두 침묵**(fail-closed 보장 붕괴). producer 테스트의 curl stub이 본문을 통째로 소비해 이 모드가 미검증 | **Accept**(수정 수용) | 하트비트를 **페이로드 마지막 줄**로 이동 = post-commit 마커. 절단은 접두부만 남기므로 **하트비트가 먼저 유실 → Stale 발화**로 fail-closed 복원. Codex가 제시한 "요청 2개 분리"는 curl 예산이 2배가 되어 `N_MAX` 7→5로 떨어지므로 **미채택**. 순서 락 @test + **절단 시나리오 @test**(stub이 접두부만 기록 → 하트비트 부재·카운트 생존 단언) 추가 |
| R-3 | medium | `Scrape alert points to logs that discard the affected app and cause` — skopeo stderr를 `/dev/null`로 버리고 빈 digest는 조용히 `continue` → 알림이 "Job 로그 확인"을 지시하는데 정작 어느 앱이 왜 실패했는지 로그에 없다 | **Accept** | 실패 시 `digest scrape failed: app=… ref=…`를 stderr로(자격증명 미노출, `key=value` 형태로 모드 C 추출기 오인 회피). producer 테스트가 실패 앱 식별·무고 앱 부재·자격증명 미유출을 단언 |
| R-4 | low | `New Alertmanager title mappings will not reach the running pod` — ConfigMap을 initContainer가 파드 기동 시에만 복사하고 checksum/reloader가 없어 기존 파드가 옛 설정을 무기한 사용(레포 기존 함정) | **Accept(운영 절차)** | 랜딩 후 체크포인트에 `kubectl rollout restart deploy/alertmanager -n observability` + 적용 확인 스텝 추가. 리로더/checksum 도입은 **F-8** |
| R-2 | medium | `Missing verification: the 60-second startup budget is not enforced` — `DEB_POD_START_BUDGET_S=60`은 매니페스트 파생이 아닌 **가정**인데 `N_MAX`와 L7 증명이 여기 의존. Codex도 "현재 용량(N=2)에선 차단 사유가 아니라 후속 등급"이라 명시 | **Defer → F-7** | 가정임을 lib 주석에 명시(N=2 여유 60초: `60+2×10+30+10=120 < 180`). startup 초과 시 Job이 죽어 **하트비트 미발행 → Stale 발화**(fail-closed — 최악은 거짓 페이지지 침묵이 아니다) |

### Codex Structure Review — r1: clean — **approve**(발견 0). skeleton(I-1) 구조 게이트 통과(2026-07-13)

아티팩트: `docs/reviews/digest-exporter-stale/structure-r1.json`(reviewedSha `7fb3a76`). 판정: 레지스트리·
producer 하네스·r4 룰 위치·replay 레그 seam·예산 헬퍼가 **I-2(카운트 + L4/L6)를 구조적 재작업 없이**
지탱한다. 테스트가 내부 구현이 아니라 기록된 공개 seam을 검증한다.

### Codex Plan Review — r10: clean — **approve**(발견 0). plan 게이트 통과(2026-07-13)

아티팩트: `docs/reviews/digest-exporter-stale/plan-r10.json`. r1~r9에서 제기된 9건(P-1~P-3 · P-2 잔여
4회 · N-1~N-5) 중 8건 Accept·1건 Reject(r7, 보상 통제 + owner waive). 재제기 없음.
