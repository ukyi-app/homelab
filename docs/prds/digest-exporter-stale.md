...
| skopeo `--command-timeout` | **10s** (env `SKOPEO_TIMEOUT` 기본값) | 앱당 GHCR 조회 상한(네트워크 블랙홀 대비). env 오버라이드는 **producer 행위 테스트가 빠른 타임아웃 케이스를 돌리기 위한 seam**이다(CronJob은 기본값 사용) |
...
| **`APPS` 카디널리티 상한** | **`N ≤ N_MAX`**, `N_MAX = ⌊(activeDeadlineSeconds − CURL_MAX_TIME − STARTUP_MARGIN(30s)) / SKOPEO_TIMEOUT⌋` = **12**(현재 값 기준; 실제 앱 2) | ⚠️ **순차 스크레이프 예산**(plan 게이트 r8): 루프가 앱을 순차 조회하므로 **N × skopeo 타임아웃 + curl 상한**이 Job 데드라인을 넘으면 **Job이 push 전에 죽는다** → 하트비트·카운트가 미발행 → GHCR 장애가 `DigestExporterScrapeIncomplete`가 아니라 `DigestExporterStale`로 **오귀속**되고 US2가 깨진다. bats 정적 게이트가 이 부등식을 매니페스트에서 파생해 강제한다 — 13번째 앱을 추가하는 순간 CI가 red가 되어 `activeDeadlineSeconds` 상향(및 아래 부트스트랩 부등식 재확인)을 강제한다 |

**강제된 최악 첫 하트비트 상한** = 크론 경계(≤600s) + 파드 스케줄·기동 예산(60s, 이미지는 digest 핀
+ 노드 캐시) + `activeDeadlineSeconds`(180s) = **840s = 14분** < `for: 900s`(여유 60s). `Replace` 덕에
이 상한은 **랜딩 시점에 구 Job이 돌고 있든 없든 성립**한다(구 Job은 다음 크론 경계에 교체된다).
`activeDeadlineSeconds`를 올리면 이 부등식이 먼저 깨지므로(preflight ①) 카디널리티 확장은 **두 부등식을
동시에** 만족해야 한다 — 그것이 이 계약의 안전장치다.
...
- **preflight 산술(전부 매니페스트에서 파생 — 하드코딩 금지)**: `for`(15m)가 eval(30s)의 정수배 ✓ ·
  ...
  **부트스트랩 불변식 4개**(digest-exporter.yaml에서 `yq`로 읽어 강제, 위반 = **exit 2 HARNESS FAULT**):
  ...
  ④ **순차 스크레이프 예산**: `N_apps × SKOPEO_TIMEOUT + CURL_MAX_TIME + STARTUP_MARGIN(30s) ≤ activeDeadlineSeconds`
  (= Job이 push 전에 죽지 않는다 — 죽으면 GHCR 장애가 `Stale`로 오귀속된다).
...
- **정적 gate 단언**(`tests/gates/test_digest-exporter.bats`, e2e와 독립): CronJob이
  ...
  누가 되돌리거나 13번째 앱을 추가하면 required gate가 red가 된다.
- **랜딩 후 라이브 체크포인트**(hermetic 하네스가 도달 못 하는 층 — 실물 핀된 k3s에서 관측).
  ...
  **실패 처리**: 첫 하트비트가 상한(840s)을 넘거나 알림이 발화하면 → **룰만 revert하는 PR**(롤백)을 내고
  원인(예: 이미지 pull 지연·스케줄러 지연)을 기록한 뒤 상한을 재산정한다. exporter 메트릭은 무해하므로
  남겨 둔다.
