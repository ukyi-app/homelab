...
feature 브랜치 안에서 슬라이스를 나누는 것은 **배포를 나누는 것이 아니다**(r2가 잡은 결함). 따라서
이 기능은 파이프라인 관례(브랜치 1개 → 랜딩 1회)를 **의도적으로 이탈해 두 번 랜딩**한다:

| 랜딩 | 내용 | 게이트 | 랜딩 후 |
|---|---|---|---|
| **랜딩 1 (PR-A)** = skeleton 슬라이스 | exporter `run.sh` 메트릭 3개 + `DEFAULT_REGISTRY` 등재 + **producer 행위 테스트**(stub skopeo + curl 캡처) | structure 게이트(skeleton diff) | `main` 머지 → ArgoCD 싱크 → **크론 1주기(≤10분) 뒤 vmsingle에서 하트비트·카운트 3개의 가시성을 라이브로 확인**(rollup 질의) |
| **랜딩 2 (PR-B)** | 알림 룰 2건 + 발화 e2e 6레그 + 함정 원장 tie + alertmanager 한국어 제목 매핑 | verification(capturing-evidence) → release 게이트 | `main` 머지 → vmalert `configCheckInterval=30s` reload. 이 시점에 하트비트는 이미 **신선**하므로 `absent`도 `time()-hb > 1200`도 거짓 → **거짓 페이지 0** |
...
- **랜딩 1과 랜딩 2 사이의 차단 조건**: 라이브 하트비트 확인 전에는 PR-B를 열지 않는다. 확인 증거
  (질의 + 출력)는 verification 단계의 `docs/reviews/digest-exporter-stale/verification.md`에 적재한다.
- **파이프라인 상태 연속성**: PRD frontmatter가 계속 SSOT다. `skeleton-issue` = 슬라이스 1이고
  `structure-gate: done`은 **PR-A 랜딩 전에** 확정된다(구조 게이트는 브랜치 diff를 보므로 랜딩과 무관).
  PR-A 랜딩 후 작업 브랜치를 **갱신된 `main`에서 다시 따서** 슬라이스 2를 얹고, release 게이트는 그
  브랜치 diff(= 슬라이스 2)를 본다. 두 랜딩 모두 `landing` 스킬이 수행한다.
...
**해소**: 배포 경계가 `main` 머지뿐이라는 사실을 계약으로 명시하고, 이 기능을 **2회 랜딩**(PR-A =
skeleton producer → 라이브 하트비트 확인 → PR-B = 룰·e2e)으로 재설계했다. 파이프라인 관례(랜딩 1회)를
의도적으로 이탈하는 것이며, 상태 연속성·게이트 배치·롤백 역순을 Implementation Decisions의
"롤아웃·롤백 계약"에 못박았다. owner가 하드룰 4의 (b) 수동 round 3을 승인했다.
