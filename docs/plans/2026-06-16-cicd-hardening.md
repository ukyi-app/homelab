...
**⚠️ #53 리베이스(2026-06-17) — 게이트/테스트 모델 갱신 (아래 규칙이 개별 섹션의 옛 `tools/test/*.bats 글롭` 서술보다 우선):**
이 플랜은 main이 #53(`b9c17bd` — 디렉토리·테스트조직 8-워크스트림 리팩터)로 진행된 뒤 그 위로 rebase됐다. #53은 **CI/CD
로직 무변경**(모든 버그 그대로)이나 경로·게이트모델이 바뀌었다.
- **게이트 테스트 수집 = 단일 SSOT `scripts/run-bats.sh`**(ci.yaml `gate` 잡 line 49 + `make ci`가 공통 호출):
  git-tracked `*test_*.bats` 전부 − `platform/charts/*` − `tests/.ci-exclude` 등록분.
- **신규 테스트**는 ① 파일명 `test_<x>.bats`(`test_` 접두 — `tests/gates/test_bats-naming.bats`가 강제) ② 위치는
  tool/스크립트-로직→`tools/tests/`, 워크플로/CI/게이트-와이어링→`tests/gates/`(둘 다 root에서 2-deep이라 in-test
  `$BATS_TEST_DIRNAME/../..`=root 보존) ③ `tests/.ci-exclude` 미등록(CI-safe)이어야 **required gate에 자동 포함**
  — ci.yaml/Makefile 별도 배선 불필요. 수집 확인: `./scripts/run-bats.sh --list | grep test_<x>`.
- **게이트 제외**는 `tests/.ci-exclude` 한 곳이 SSOT(terraform/live/docker/age 의존만; `test_bats-accounting.bats`가
  사유+실행처 주석 강제). CI-safe(age 키 불필요) 테스트는 여기 넣지 말 것.
- **HARD GATE 지뢰 3종(self-fail 주의):** ① `test_` 미접두 *.bats → `test_bats-naming` 실패 ② 리네임된 워크플로의
  `.yml` 기저명을 docs/plans 밖(테스트본문·here-doc)에 남기면 `test_workflow-yaml.bats`(3번째 @test) 실패 → 전부
  `.yaml` ③ telegram 콜사이트 총계는 **현재 15** — 새 콜사이트(P3 dns-drift·P6 pr-sweeper·P8 build.yaml)는 here-doc
  줄 추가 + 정수 +1(절대값 16/17 prebake 금지 — 머지 순서에 따라 다름, 머지 시점 현재값 +1로 갱신).
...
3. **Phase 4 supplychain-7의 행동(decoy 거부) 회귀 테스트는 게이트 포함 파일에 둔다.** `tests/test_sops-guard.bats`는 **`tests/.ci-exclude` 등재**로 run-bats.sh 수집에서 제외된다(실 age 키 의존 — verify.yaml ephemeral 키가 대체 커버; ci.yaml 하드코딩 라인이 아니라 `.ci-exclude` 한 곳이 SSOT). 따라서 age 키 불필요한 decoy/partial-enc 거부 **행동** 단언을 `tests/gates/test_gate-secret-guard.bats`(CI-safe → 자동 게이트)에 추가한다. 구조 검증만 verify 잡(비필수)에 남긴다.

4. **Phase 5는 단일 원자 PR — RED-only 커밋 금지.** `tests/gates/test_auth.bats`는 게이트 포함이라 구현 없는 standalone RED 커밋은 required CI를 깬다. **테스트 작성 + 구현(tf 리소스/변수 제거)을 한 커밋에 묶는다.** 라이브 DEPLOY_BOT_PAT 삭제(owner-local `terraform -chdir=infra/github apply`)는 코드 게이트 밖 수동 태스크임을 명시(인지된 non-TDD).
...
라이브 버그 하나(obs-1)와 그 근본원인을 양방향으로 잠그는 공유 게이트 bats(obs-2)를 다룬다. 두 발견은 같은 파일(`.github/actions/telegram-notify/notify.sh:25`의 enum 건초더미)을 중심으로 한다. obs-2가 만드는 `tests/gates/test_telegram-source-enum.bats`는 설계 공유인프라 #5(notify source-label 검증 bats)이며 이후 단계는 이를 참조만 한다.
...
- `tf-reconcile.yaml:163,225`가 `source: IaC드리프트`를 발화하는데 `notify.sh:25` 건초더미엔 `IaC IaC수렴`만 있고 `IaC드리프트`가 없다 → `*)` 기본 분기 `exit 2`(라이브 침묵 + run red).
- 게이트 글롭은 `ls tools/test/*.bats`(Makefile:104, ci.yaml:45) — 신규 `tests/gates/test_telegram-source-enum.bats`는 자동 포함.
...
- Test `/Users/ukyi/workspace/homelab-cicd-hardening/tests/gates/test_telegram-notify.bats` (기존 파일에 회귀 케이스 추가)
...
```
bats tests/gates/test_telegram-notify.bats
```
기대 실패: 새 케이스가
```
✗ accepts the IaC드리프트 source label emitted by tf-reconcile drift steps (obs-1 live bug)
   (in test file tests/gates/test_telegram-notify.bats, line N)
     `[ "$status" -eq 0 ]' failed
```
...
**Step 5: Commit** —
```
git -C /Users/ukyi/workspace/homelab-cicd-hardening add .github/actions/telegram-notify/notify.sh tests/gates/test_telegram-notify.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "fix: notify.sh enum에 IaC드리프트 추가 — tf-reconcile 드리프트 알림 침묵 해소"
```
...
**Files:**
- Create `/Users/ukyi/workspace/homelab-cicd-hardening/tests/gates/test_telegram-source-enum.bats` (공유 #5 — notify source-label bats)
...
bats tests/gates/test_telegram-source-enum.bats
```
기대 실패:
```
...
**Step 5: Commit** —
```
git -C /Users/ukyi/workspace/homelab-cicd-hardening add tests/gates/test_telegram-source-enum.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "test: 워크플로 source↔notify enum 양방향 게이트 — obs-1 근본원인 차단"
```
...
- Modify (extend) Test `tools/tests/test_bump.bats`

**Step 1: Write the failing test** — `tools/tests/test_bump.bats`에 아래 `@test`들을 파일 끝(line 81 `}` 다음)에 추가한다. `DIG` 변수(line 33)는 기존 정의를 재사용한다.
...
```bash
eval "$(mise activate bash)"; bats tools/tests/test_bump.bats
```
...
기대 출력: 모든 `@test ... ok`(기존 8 + 신규 5 = 13 통과 — unknown-flag·expect-current 불일치·expect-current 일치·arity 값없음·arity 플래그-as-값), `13 tests, 0 failures`.

**Step 5: Commit**

```bash
git -C /Users/ukyi/workspace/homelab-cicd-hardening add tools/bump-tag.mjs tools/tests/test_bump.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "fix: bump-tag 오타 플래그 거부 + --expect-current TOCTOU 가드 (digest 핀 격하 차단)"
```
...
- Modify (extend) Test `tools/tests/test_cli-flag-guard.bats`

**Step 1: Write the failing test** — 기존 `tools/tests/test_cli-flag-guard.bats`(line 25 마지막 `@test` 다음)에 추가한다. 이 파일의 `setup()`은 repo root로 cd한다(line 7).
...
```bash
eval "$(mise activate bash)"; bats tools/tests/test_cli-flag-guard.bats
```
...
```bash
eval "$(mise activate bash)"; bats tools/tests/test_cli-flag-guard.bats && bats tools/tests/test_teardown.bats
```
...
```bash
git -C /Users/ukyi/workspace/homelab-cicd-hardening add tools/teardown-app.mjs tools/teardown-resource.mjs tools/tests/test_cli-flag-guard.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "fix: teardown-app/teardown-resource 오타 플래그 거부 (mutator fail-closed 균일화)"
```
...
- Test `tools/tests/test_identity.bats` (Create)

**Step 1: Write the failing test** — `tools/tests/test_identity.bats`를 새로 만든다(`tools/test/*.bats` 글롭이 gate에 자동 포함). `@test` 이름은 영어, 중간 단언은 `[ ]`. node 인라인으로 export된 정규식과 6개 콜사이트의 동작 일치를 검증한다.
...
```bash
eval "$(mise activate bash)"; bats tools/tests/test_identity.bats
```
...
```bash
eval "$(mise activate bash)"; bats tools/tests/test_identity.bats && bats tools/tests/test_create-app.bats tools/tests/test_onboard.bats tools/tests/test_teardown.bats tools/tests/test_validate-mutation.bats tools/tests/test_activate-app.bats tools/tests/test_bump.bats
```
...
```bash
git -C /Users/ukyi/workspace/homelab-cicd-hardening add tools/lib/identity.mjs tools/create-app.mjs tools/onboard-app.mjs tools/teardown-app.mjs tools/validate-mutation.mjs tools/activate-app.mjs tools/bump-tag.mjs tools/tests/test_identity.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "refactor: 앱-이름 regex SSOT(tools/lib/identity.mjs)로 6 콜사이트 수렴"
```
...
기대: `make ci`가 gate 8스텝(skeleton + 원장 conftest + sops 라운드트립 + bats 스위트 + shellcheck 등)을 통과. 신규 `tools/tests/test_identity.bats`는 `bats tools/test/*.bats` 글롭(Makefile:104)이 자동 포함하므로 별도 배선 불필요.
...
> `tests/test_cloudflare-entitlement.bats`에 둬야 required gate가 강제한다. drift-2 reconcile 구조 단언도 동일 이유로
> 기존 `infra/_tests/test_tf_reconcile.bats`(비-required)를 보강하되, 핵심 회귀는 `tests/`로 끌어올린다.
...
- Test: `tools/tests/test_tf-destroy-guard.bats` (gate 글롭 `tools/test/*.bats`가 자동 포함)
...
`tools/tests/test_tf-destroy-guard.bats`:
```bash
#!/usr/bin/env bats
...
**Step 2: Run it, expect FAIL**
```
bats tools/tests/test_tf-destroy-guard.bats
```
...
**Step 5: Commit**
```
git -C /Users/ukyi/workspace/homelab-cicd-hardening add .github/actions/tf-destroy-guard/action.yml .github/actions/tf-destroy-guard/destroy-guard.sh tools/tests/test_tf-destroy-guard.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "feat: tf-destroy-guard composite — tf plan destroy-count 가드 SSOT(warn|block)"
```
...
- Modify: `.github/workflows/tf-reconcile.yaml:77-86` (reconcile: 인라인 block jq → composite mode=block)
- Test: `infra/_tests/test_tf_reconcile.bats` (보강 — reconcile이 composite를 쓰는지) +
  Create: `tests/test_iac-destroy-guard.bats` (required gate 글롭 `tests/*.bats`로 picked up)

> **Notes:** drift-1의 핵심 회귀("primary apply 경로에 destroy 가드 부재")는 required gate에서 강제돼야 한다.
> 그래서 apply job 구조 단언은 `tests/test_iac-destroy-guard.bats`에 둔다(비-required `iac-validate`가 아니라).
> tf-reconcile composite 채택 단언은 기존 `infra/_tests/test_tf_reconcile.bats`를 보강하되, 기존
...
`tests/test_iac-destroy-guard.bats`:
```bash
#!/usr/bin/env bats
...
기존 `infra/_tests/test_tf_reconcile.bats`에 reconcile composite 채택 단언 추가:
```bash
...
**Step 2: Run it, expect FAIL**
```
bats tests/test_iac-destroy-guard.bats infra/_tests/test_tf_reconcile.bats
```
...
`tf-reconcile.yaml` reconcile — 77-86의 인라인 destroy jq를 composite로 치환. case `2)` 분기 구조상
...
기존 `infra/_tests/test_tf_reconcile.bats`의 `cloudflare reconcile keeps the destroy guard` 테스트는 인라인
...
```
command -v actionlint >/dev/null && actionlint .github/workflows/iac.yaml .github/workflows/tf-reconcile.yaml || true
```

**Step 5: Commit**
```
git -C /Users/ukyi/workspace/homelab-cicd-hardening add .github/workflows/iac.yaml .github/workflows/tf-reconcile.yaml tests/test_iac-destroy-guard.bats infra/_tests/test_tf_reconcile.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "fix: iac.yaml apply에 destroy 가드 삽입 + 3곳을 tf-destroy-guard composite로 수렴"
```
...
**Files:**
- Create: `tests/test_cloudflare-entitlement.bats` (required gate 글롭 `tests/*.bats`로 picked up)
...
`tests/test_cloudflare-entitlement.bats`:
```bash
#!/usr/bin/env bats
...
bats tests/test_cloudflare-entitlement.bats
```
...
코드 변경 없음 — `tests/test_cloudflare-entitlement.bats`가 곧 구현(현재 `waf.tf`/`cache.tf`가 이미 적법:
...
**Step 5: Commit**
```
git -C /Users/ukyi/workspace/homelab-cicd-hardening add tests/test_cloudflare-entitlement.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "test: Cloudflare 무료 플랜 entitlement 정적 게이트(period/mitigation==10·matches 금지)"
```
...
**Files:**
- Modify: `.github/workflows/tf-reconcile.yaml` (reconcile job: delete 발견 시 hard-exit → ::warning:: + telegram, **apply 전체 skip[원자적]**, 잡은 생존; owner 로컬 apply 후 다음 주기 수렴)
- Test: `infra/_tests/test_tf_reconcile.bats` (보강) + `tests/test_iac-destroy-guard.bats` (drift-2 구조 단언 추가)
...
`infra/_tests/test_tf_reconcile.bats` 보강:
```bash
...
`tests/test_iac-destroy-guard.bats`에 drift-2 분리 불변식 추가:
```bash
...
**Step 2: Run it, expect FAIL**
```
bats infra/_tests/test_tf_reconcile.bats tests/test_iac-destroy-guard.bats
```
Expected: `reconcile guard step is continue-on-error and emits a warning` 실패 —
`continue-on-error: true` 가 tf-reconcile.yaml에 아직 없어 status≠0. `reconcile telegram fires on
delete-blocked drift`도 `guard|outcome` 미존재로 실패.

**Step 3: Minimal implementation**

`tf-reconcile.yaml` reconcile job — Task 2에서 만든 가드 스텝을 `continue-on-error: true` + `id`로 바꾸고,
...
선택 워크플로 문법 점검:
```
command -v actionlint >/dev/null && actionlint .github/workflows/tf-reconcile.yaml || true
```

**Step 5: Commit**
```
git -C /Users/ukyi/workspace/homelab-cicd-hardening add .github/workflows/tf-reconcile.yaml infra/_tests/test_tf_reconcile.bats tests/test_iac-destroy-guard.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "fix: tf-reconcile destroy 가드를 alert-and-skip로 — 합법 teardown 영구 차단 해소"
```
...
- Create: `.github/workflows/dns-drift.yaml` (스케줄 opt-in, preflight skip, checkout→체커→telegram)
- Create (test): `tools/tests/test_dns-drift-check.bats` (fixture resolver: NXDOMAIN→drift / 전부 resolve→무드리프트)

> ⚠️ 횡단 조정: `dns-drift.yaml`은 `./.github/actions/telegram-notify`를 쓰는 **새 콜사이트**다 — checkout이
...
**Step 1: Write the failing test** — `tools/tests/test_dns-drift-check.bats`:
```bash
#!/usr/bin/env bats
...
**Step 2: Run it, expect FAIL** — `bats tools/tests/test_dns-drift-check.bats`
...
**Step 4: Run test, expect PASS** — `bats tools/tests/test_dns-drift-check.bats` → `3 tests, 0 failures`(NXDOMAIN→drift / 전부 resolve→무 / transient→별도 버킷).

**Step 5: 워크플로 + Commit** — `.github/workflows/dns-drift.yaml`:
```yaml
...
make ci
```
Expected: 신규 `tools/tests/test_tf-destroy-guard.bats`(글롭)·`tests/test_cloudflare-entitlement.bats`·
`tests/test_iac-destroy-guard.bats`·`tests/test_iac-live-dns-note.bats`(글롭) + 갱신된 `shellcheck`(destroy-guard.sh) 포함
전 스텝 통과, exit 0. `iac-validate`의 `infra/_tests/test_tf_reconcile.bats`도 PR(iac.yaml paths) 시 통과.
...
>
> **배경 사실(라이브 확인됨)**: required check는 `gate`(ci.yaml의 잡) 단일이다 — `verify`(verify.yaml,
...
> `tests/test_sops-guard.bats`를 확장하고(이 파일은 `make ci`/로컬에서 실행되며 실 age 키 의존 케이스가
...
- Test: `tests/test_sops-guard.bats` (기존 3 케이스 유지 + 데코이 우회 회귀 케이스 추가)

**Step 1: Write the failing test** — `tests/test_sops-guard.bats`에 데코이 케이스를 추가한다(기존 setup/teardown 재사용).
...
```bash
bats tests/test_sops-guard.bats
```
...
```bash
git -C /Users/ukyi/workspace/homelab-cicd-hardening add scripts/sops-guard.sh tests/test_sops-guard.bats && \
...
- Test: `tests/gates/test_gate-secret-guard.bats` (게이트 잡이 gitleaks 핀 + sops-guard를 강제하는지 정적 단언 — run-bats.sh가 자동 수집(`test_*.bats`, CI-safe → required gate))

**Step 1: Write the failing test** — `tests/gates/test_gate-secret-guard.bats` 신규:
...
```bash
bats tests/gates/test_gate-secret-guard.bats
```
...
```bash
git -C /Users/ukyi/workspace/homelab-cicd-hardening add .github/workflows/ci.yaml Makefile tests/gates/test_gate-secret-guard.bats && \
...
- Test: `tests/gates/test_branch_protection.bats` (신규 — **run-bats.sh가 자동 수집(`test_*.bats`, CI-safe → required gate)**)
...
> 둔다. iac.yaml 배선은 불필요(글롭이 자동 포함). Phase 5 Task 1이 같은 이유로 `tests/gates/test_auth.bats`를 쓴다.

**Step 1: Write the failing test** — `tests/gates/test_branch_protection.bats` 신규(`$BATS_TEST_DIRNAME` 상대경로 + grep 불변식):
...
```bash
bats tests/gates/test_branch_protection.bats
```
...
**Step 4: Run test, expect PASS** — `tests/gates/test_branch_protection.bats`는 `gate` 잡 글롭(`tools/test/*.bats`)으로
...
```bash
git -C /Users/ukyi/workspace/homelab-cicd-hardening add infra/github/repo.tf tests/gates/test_branch_protection.bats && \
...
> `terraform.tfvars.example`(주석)·`.env.secrets.example`(`TF_VAR_bot_pat`)·`tf-reconcile.yaml:135`
...
> `variable "bot_pat"` 제거 후 `tf-reconcile.yaml`에 `TF_VAR_bot_pat`가 남아도 plan은 안 깨진다 — 그래도
...
가드는 기존 `tests/gates/test_auth.bats`(이미 PAT-0 불변식 파일)에 추가한다.

**Files:**
- Modify (test): `tests/gates/test_auth.bats` (현재 4개 `@test`, line 1~28에 추가)

**Step 1: Write the failing test** — `tests/gates/test_auth.bats` 끝(line 28 `}` 다음)에 4개 `@test` 추가.
...
**Step 5: Commit (PR-내 중간 커밋 — 단독 push 금지)** — ⚠️ 횡단 조정 #4: `tests/gates/test_auth.bats`는 `gate`
...
```
git -C /Users/ukyi/workspace/homelab-cicd-hardening add tests/gates/test_auth.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "test: DEPLOY_BOT_PAT terraform 부재 회귀 가드 (supplychain-2, PR 최종에 green)"
```
...
**Step 1: Write the failing test** — Task 1에서 작성 완료(`tests/gates/test_auth.bats`의 신규 4건). 이 Task는
...
**Step 4: Run test, expect PASS** —
```
bats tests/gates/test_auth.bats && make tf-validate
```
...
```
bats tests/gates/test_auth.bats -f 'bot_pat resource'
bats tests/gates/test_auth.bats -f 'variable bot_pat'
bats tests/gates/test_auth.bats -f 'DEPLOY_BOT_PAT secret_name'
```
세 건 모두 `ok`.
...
### Task 3: dead bot_pat 참조 정리 — tfvars.example·.env.secrets.example·tf-reconcile.yaml (supplychain-2)

변수가 사라졌으니 그것을 채우던 주입/문서 참조를 정리한다. `tf-reconcile.yaml`의 `TF_VAR_bot_pat` 주입은
...
**Files:**
- Modify: `.github/workflows/tf-reconcile.yaml:135` (`TF_VAR_bot_pat:` 라인 삭제)
...
**Step 2: Run it, expect FAIL** —
```
bats tests/gates/test_auth.bats -f 'TF_VAR_bot_pat'
bats tests/gates/test_auth.bats -f 'DEPLOY_BOT_PAT secret_name'
```
기대 FAIL:
```
...
```
(tf-reconcile.yaml:135에 `TF_VAR_bot_pat` 존재 + terraform.tfvars.example:11에 `DEPLOY_BOT_PAT` 언급 존재.)

**Step 3: Minimal implementation** —

(a) `.github/workflows/tf-reconcile.yaml`의 drift-github 잡에서 line 135 한 줄 삭제:
```yaml
...
- line 11~13 블록의 `write-only 시크릿(DEPLOY_BOT_PAT 등)은 ...` 문장 삭제(이제 write-only 시크릿이
  ...
  ```
  # 선택: tf-reconcile.yaml의 plan-only 드리프트 알림을 켜려면 아래 Actions 시크릿을 등록한다
...
**Step 4: Run test, expect PASS** —
```
bats tests/gates/test_auth.bats && make tf-validate && shellcheck $(git -C /Users/ukyi/workspace/homelab-cicd-hardening ls-files '*.sh') >/dev/null
```
...
```
python3 -c "import yaml,sys; yaml.safe_load(open('/Users/ukyi/workspace/homelab-cicd-hardening/.github/workflows/tf-reconcile.yaml'))" && echo yaml-ok
```

**Step 5: Commit** —
```
git -C /Users/ukyi/workspace/homelab-cicd-hardening add .github/workflows/tf-reconcile.yaml infra/github/terraform.tfvars.example .env.secrets.example && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "refactor: dead bot_pat 주입·문서 참조 정리 — tf-reconcile/tfvars/env.secrets (supplychain-2)"
```
...
2. 다음 `tf-reconcile.yaml` 주기(30분) 또는 수동 `workflow_dispatch`에서 `drift-github` 잡이 `terraform
...
```
bats /Users/ukyi/workspace/homelab-cicd-hardening/tests/gates/test_auth.bats
```
기대: 8/8 `ok`.
...
- Modify (test) `tools/tests/test_bump.bats:76-81` (기존 "single concurrency group" 단언 강화)

**Step 1: Write the failing test** — `tools/tests/test_bump.bats`의 마지막 `@test`를 정확한 group/queue 단언으로 교체한다. 기존(76-81)은 group 비-공백 + cancel-in-progress=false만 본다.
...
**Step 2: Run it, expect FAIL** — `bats tools/tests/test_bump.bats`
기대 실패: 마지막 테스트가 `(in test file tools/tests/test_bump.bats, line 78) \`[ "$output" == "homelab-mutation" ]' failed` — 현재 group은 `values-writeback`이고 `.concurrency.queue`는 `null`.
...
**Step 4: Run test, expect PASS** — `bats tools/tests/test_bump.bats`
기대 출력: `ok N bump workflow joins the global homelab-mutation queue (no pending loss)` 포함 전체 통과(`N tests, 0 failures`). 워크플로 YAML 파싱 게이트도 무영향: `bats tests/gates/test_workflow-yaml.bats` 통과.

**Step 5: Commit**
`git -C /Users/ukyi/workspace/homelab-cicd-hardening add .github/workflows/bump.yaml tools/tests/test_bump.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "fix: bump 워크플로를 homelab-mutation+queue:max 전역 직렬화에 합류 (write-back 유실 차단)"`
...
- Create (test) `tests/gates/test_automerge-fallback.bats`

**Step 1: Write the failing test** — `tests/gates/test_automerge-fallback.bats`. `gh`를 PATH stub으로 대체해(`mergeStateStatus`/머지 호출을 기록) 분기 행동을 검증한다. raw 직접 머지는 mergeStateStatus가 CLEAN일 때만 일어나야 하고, BLOCKED면 시끄럽게 실패해야 한다.
...
**Step 2: Run it, expect FAIL** — `bats tests/gates/test_automerge-fallback.bats`
...
**Step 4: Run test, expect PASS** — `bats tests/gates/test_automerge-fallback.bats`
기대 출력: `5 tests, 0 failures`. shellcheck 게이트도 통과: `shellcheck scripts/auto-merge-or-fail.sh` → 출력 없음(exit 0).

**Step 5: Commit**
`git -C /Users/ukyi/workspace/homelab-cicd-hardening add scripts/auto-merge-or-fail.sh tests/gates/test_automerge-fallback.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "feat: CLEAN 판별 auto-merge fallback 공유 스크립트 (분기보호 우회 직접머지 차단)"`
...
- Modify `.github/workflows/bump-poll.yaml:107`
- Modify `.github/workflows/_create-database.yaml:90`
- Modify `.github/workflows/_create-cache.yaml:92`
- Modify `.github/workflows/_update-secrets.yaml:88`
- Create (test) — Task 2의 `automerge-fallback.bats`에 콜사이트 수렴 단언 추가

**Step 1: Write the failing test** — `tests/gates/test_automerge-fallback.bats`에 6콜사이트가 raw fallback(`gh pr merge --auto --squash "$branch" || gh pr merge --squash "$branch"`)을 더는 쓰지 않고 공유 스크립트를 부르는지 단언하는 테스트를 추가한다.
...
bump-poll.yaml:107은 들여쓰기가 한 단계 깊다(while 루프 내부) — 매칭 들여쓰기로:
...
**Step 4: Run test, expect PASS** — `bats tests/gates/test_automerge-fallback.bats && bats tests/gates/test_workflow-yaml.bats`
기대 출력: automerge-fallback 전체 통과(6 tests, 0 failures), workflow-yaml YAML 파싱 통과. telegram-callsites 회귀 없음: `bats tests/gates/test_telegram-callsites.bats` 통과.

**Step 5: Commit**
`git -C /Users/ukyi/workspace/homelab-cicd-hardening add .github/workflows/bump.yaml .github/workflows/bump-poll.yaml .github/workflows/_create-database.yaml .github/workflows/_create-cache.yaml .github/workflows/_update-secrets.yaml tests/gates/test_automerge-fallback.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "refactor: 6개 auto-merge 콜사이트를 공유 fallback 스크립트로 수렴"`
...
**Files:**
- Create `.github/workflows/pr-sweeper.yaml`
- Create (test) `tests/gates/test_pr-sweeper.bats`

**Step 1: Write the failing test** — `tests/gates/test_pr-sweeper.bats`. 워크플로 YAML을 정적 검사: 스케줄 트리거, 봇 PR(bump*/create-*/onboard-*/update-secrets*) 한정, BEHIND PR에 `gh pr update-branch`, 최소 권한, 실패 알림(source: 변이).
...
**Step 2: Run it, expect FAIL** — `bats tests/gates/test_pr-sweeper.bats`
기대 실패: 모든 테스트가 파일 부재로 실패 — `yq`가 빈 파일/없는 파일에서 `null`을 내거나 grep이 매치 0 → 첫 `@test`부터 `(line 17) [ -n "$output" ]` 또는 grep 실패.

**Step 3: Minimal implementation** — `.github/workflows/pr-sweeper.yaml`:
...
**Step 4: Run test, expect PASS** — `bats tests/gates/test_pr-sweeper.bats && bats tests/gates/test_workflow-yaml.bats`
...
**Step 5: Commit**
`git -C /Users/ukyi/workspace/homelab-cicd-hardening add .github/workflows/pr-sweeper.yaml tests/gates/test_pr-sweeper.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "feat: stale auto-merge-pending PR 스위퍼 워크플로 추가 (BEHIND 봇 PR update-branch 수렴)"`
...
**Files:**
- Modify (test) `tests/gates/test_telegram-callsites.bats:10-33` (열거 목록 + 합계)
...
**Files:**
- Modify `.github/workflows/_create-database.yaml:91-99` (telegram body)
- Modify `.github/workflows/_create-cache.yaml:93-101`
- Modify `.github/workflows/_update-secrets.yaml:89-97`
- Modify `.github/workflows/bump.yaml:112-119` 및 `:198-205`
- Create (test) — `pr-sweeper.bats`에 알림 시맨틱 단언 추가(파일 정합 검사)

**Step 1: Write the failing test** — `tests/gates/test_pr-sweeper.bats`에 추가: auto-merge 경로 워크플로의 telegram body가 "배포됨/완료" 같은 종결 어휘 대신 "머지 대기"를 담아 obs-5(무장 ≠ 배포)를 표면화하는지 검사.
...
**Step 2: Run it, expect FAIL** — `bats tests/gates/test_pr-sweeper.bats`
기대 실패: `missing '머지 대기' notice in _create-database.yaml` — 현재 body는 "핸들 …" 만 담는다.
...
**Step 4: Run test, expect PASS** — `bats tests/gates/test_pr-sweeper.bats && bats tests/gates/test_telegram-callsites.bats`
기대 출력: pr-sweeper 전체 통과(머지-대기 단언 포함), telegram-callsites 회귀 없음(body 키는 required set에 없으므로 "required with: keys" 단언 무영향).

**Step 5: Commit**
`git -C /Users/ukyi/workspace/homelab-cicd-hardening add .github/workflows/_create-database.yaml .github/workflows/_create-cache.yaml .github/workflows/_update-secrets.yaml .github/workflows/bump.yaml tests/gates/test_pr-sweeper.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "fix: auto-merge 알림을 'PR 생성됨(머지 대기)'로 정정 (무장≠배포 표면화)"`
...
**Files:**
- Modify `.github/workflows/bump-poll.yaml:96-99` (checkout main 후 bump-tag 호출)
- Create (test) `tools/tests/test_bump-poll-toctou.bats`

**Step 1: Write the failing test** — `tools/tests/test_bump-poll-toctou.bats`. `--expect-current` 옵션은 **Phase 2에서 bump-tag.mjs에 구축**된다(여기선 사용만). 이 테스트는 워크플로가 plan→push 사이 `git checkout main` 후 현재 tag를 재증명하도록 `--expect-current`를 넘기는지 정적 검사한다.
...
**Step 2: Run it, expect FAIL** — `bats tools/tests/test_bump-poll-toctou.bats`
기대 실패: `\`grep -q -- "--expect-current" "$F"' failed` — 현재 bump-poll은 `--expect-current`를 넘기지 않는다. F2 케이스도 `expect=...current.tag` 미발견으로 실패.

**Step 3: Minimal implementation** — `bump-poll.yaml`의 bump 루프(88-99)에서 **플래너 item의 `.current.tag`**(plan 시점에 기록된 현재 배포 tag — `poll-ghcr.mjs`가 `result.current = { tag, digest }`로 emit하며 각 plan item에 `.current.tag`로 존재)를 뽑아 `--expect-current`로 넘긴다. 96-99를 다음으로 교체:
...
**Step 4: Run test, expect PASS** — `bats tools/tests/test_bump-poll-toctou.bats && bats tests/gates/test_workflow-yaml.bats`
기대 출력: bump-poll-toctou 3 tests 0 failures, workflow-yaml YAML 파싱 통과.

**Step 5: Commit**
`git -C /Users/ukyi/workspace/homelab-cicd-hardening add .github/workflows/bump-poll.yaml tools/tests/test_bump-poll-toctou.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "fix: bump-poll 루프에 --expect-current TOCTOU 가드 배선 (stale 증명 push 차단)"`
...
- Modify (test) `tools/tests/test_activate-app.bats` (마커 기록 + 자기-무효화 회귀 단언)
...
**Step 1: Write the failing test** — `tools/tests/test_activate-app.bats`에 추가: `--flip` 통과 시 `.activation`이 증명한 sha + canonical surfaceHash를 담아 기록되고, **마커 커밋 후에도 해시가 동일**한지(자기 무효화 회귀) 검사.
...
- Modify (test) `tools/tests/test_audit-orphans.bats` (surface-drift 단언)

**Step 1: Write the failing test** — `tools/tests/test_audit-orphans.bats`에 추가: active:true 앱의 `.activation` surfaceHash가 현재 canonical surfaceHash와 다르면 `activation-surface-drift`가 **정보성으로 리포트**된다(⚠️ codex pass3 F1: `--ci` **비차단** — 차단하면 정상 이미지 bump가 데드락). fixture는 git repo가 필요(canonical 해시 계산) — 픽스처 setup을 보강하거나 별도 setup의 새 describe로.
...
**Step 2: Run it, expect FAIL** — `bats tools/tests/test_audit-orphans.bats`
...
**Step 5: Commit**
`git -C /Users/ukyi/workspace/homelab-cicd-hardening add tools/audit-orphans.mjs tools/tests/test_audit-orphans.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "feat: audit-orphans에 activation surface-drift 정보성 리포트 추가 (races-5, 비차단)"`
...
- Create (test) — `tools/tests/test_onboard.bats`에 브랜치명 단언 추가(파일 존재 확인 후 적절 위치)

**Step 1: Write the failing test** — `tools/tests/test_onboard.bats`에 추가(워크플로 정적 검사 형식). 고정 `onboard/${APP}` 금지, `${{ github.run_id }}` 포함을 강제 — 다른 모든 dispatch reusable과 동일 패턴.
...
**Step 2: Run it, expect FAIL** — `bats tools/tests/test_onboard.bats`
...
**Step 4: Run test, expect PASS** — `bats tools/tests/test_onboard.bats && bats tests/gates/test_workflow-yaml.bats`
기대 출력: onboard.bats 전체 통과(신규 브랜치명 단언 포함), workflow-yaml YAML 파싱 통과.

**Step 5: Commit**
`git -C /Users/ukyi/workspace/homelab-cicd-hardening add .github/workflows/onboard.yaml tools/tests/test_onboard.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "fix: onboard 브랜치명에 run_id 부여 (재dispatch 충돌·dangling 차단)"`
...
- `bats tools/tests/test_bump.bats tests/gates/test_automerge-fallback.bats tests/gates/test_pr-sweeper.bats tools/tests/test_bump-poll-toctou.bats tools/tests/test_activate-app.bats tools/tests/test_audit-orphans.bats tools/tests/test_onboard.bats tests/gates/test_telegram-callsites.bats tests/gates/test_workflow-yaml.bats` → 전부 통과.
...
>
> 순서 주의: **Task 1(setup-toolchain kubeseal input) → Task 2(checksum) → Task 3(dry-1/dry-2 인라인 흡수)**. dry-1이 onboard/_create-app의 인라인 helm/kubeconform/conftest curl을 제거하므로, 기존 `tests/gates/test_ci-toolchain-pin.bats`의 "인라인 helm 핀" 단언이 깨진다 → Task 3에서 그 테스트를 함께 갱신한다.
>
...
- Test `tests/gates/test_setup-toolchain-kubeseal.bats` (Create)

**Step 1: Write the failing test**

```bash
cat > tests/gates/test_setup-toolchain-kubeseal.bats <<'BATS'
#!/usr/bin/env bats
...
```bash
bats tests/gates/test_setup-toolchain-kubeseal.bats
```
...
```bash
git -C /Users/ukyi/workspace/homelab-cicd-hardening add .github/actions/setup-toolchain/action.yml tests/gates/test_setup-toolchain-kubeseal.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "feat: setup-toolchain에 kubeseal input 추가 (봉인 버전 SSOT v0.37.0)"
```
...
- Test `tests/gates/test_toolchain-checksums.bats` (Create)

**Step 1: Write the failing test**

```bash
cat > tests/gates/test_toolchain-checksums.bats <<'BATS'
#!/usr/bin/env bats
...
```bash
bats tests/gates/test_toolchain-checksums.bats
```
...
```bash
bats tests/gates/test_toolchain-checksums.bats && bats tests/gates/test_setup-toolchain-kubeseal.bats
```
...
```bash
git -C /Users/ukyi/workspace/homelab-cicd-hardening add .github/actions/setup-toolchain/action.yml tests/gates/test_toolchain-checksums.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "feat: setup-toolchain 전 다운로드에 SHA256 검증 + age 버전 핀 (공급망 위생)"
```
...
`onboard.yaml:46-51`·`_create-app.yaml:99,101,108`(인라인 kubeconform/helm/conftest), `_create-cache.yaml:54-55`·`_create-database.yaml:55-56`(인라인 kubeseal)을 `setup-toolchain` 채택으로 교체한다. 이때 cache의 v0.27.3 → v0.37.0 수렴이 자동 달성된다. 기존 `tests/gates/test_ci-toolchain-pin.bats`의 "onboard/_create-app 인라인 helm 핀" 단언이 깨지므로 같은 커밋에서 갱신한다.

**Files:**
- Modify `.github/workflows/onboard.yaml` (`install chart toolchain` step → composite)
- Modify `.github/workflows/_create-app.yaml` (인라인 kubeconform/helm curl 제거 → composite step)
- Modify `.github/workflows/_create-cache.yaml` (kubeseal+conftest 인라인 → composite)
- Modify `.github/workflows/_create-database.yaml` (kubeseal 인라인 → composite, cert 확인 step 분리 유지)
- Modify `.github/workflows/_update-secrets.yaml` (yq 사용 — composite로 yq 명시 설치; 현재 yq 설치 step 부재라 잠재 버그도 함께 차단)
- Modify `tests/gates/test_ci-toolchain-pin.bats` (인라인 helm 핀 단언 → composite 채택 단언으로 갱신)
- Test `tests/gates/test_setup-toolchain-kubeseal.bats` (Task 1 — 콜사이트 단언 확장)

**Step 1: Write the failing test**

`tests/gates/test_setup-toolchain-kubeseal.bats`에 콜사이트 단언을 추가(append):
...
그리고 `tests/gates/test_ci-toolchain-pin.bats`의 두 번째 `@test`를 갱신: onboard/_create-app가 더 이상 인라인 helm을 갖지 않고 composite를 쓰는지로 단언을 바꾼다.
...
```bash
bats tests/gates/test_setup-toolchain-kubeseal.bats tests/gates/test_ci-toolchain-pin.bats
```
...
`_create-app.yaml` — "생성 + 렌더 게이트 + PR" step의 인라인 kubeconform/helm/conftest curl(99,101,108줄)을 제거하고, 같은 step 앞에 composite step을 추가:
...
`_create-cache.yaml` — `kubeseal + conftest 설치` step(52-58)을 composite로:
...
`_create-database.yaml` — `kubeseal 설치 + 봉인 cert 확인` step(50-57)에서 cert 확인은 유지하되 설치는 composite로 분리:
...
`_update-secrets.yaml` — yq를 쓰지만(65-74줄 `yq -e`/`yq -i`) 설치 step이 없다(러너 기본 yq 의존 — 잠재 드리프트). composite로 yq 명시 설치 step을 checkout 뒤에 추가:
...
```bash
bats tests/gates/test_setup-toolchain-kubeseal.bats tests/gates/test_ci-toolchain-pin.bats tests/gates/test_workflow-yaml.bats
```
...
```bash
git -C /Users/ukyi/workspace/homelab-cicd-hardening add .github/workflows/onboard.yaml .github/workflows/_create-app.yaml .github/workflows/_create-cache.yaml .github/workflows/_create-database.yaml .github/workflows/_update-secrets.yaml tests/gates/test_setup-toolchain-kubeseal.bats tests/gates/test_ci-toolchain-pin.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "refactor: 인라인 toolchain curl을 setup-toolchain으로 흡수 + kubeseal v0.37.0 수렴"
```
...
- Modify `ci.yaml`(18-22), `onboard.yaml`(41-44), `bump.yaml`(51-54, 142-145), `bump-poll.yaml`(64-67), `_create-app.yaml`(57-60), `_create-database.yaml`(33-36), `_create-cache.yaml`(32-35), `_teardown.yaml`(36-39), `_audit.yaml`(14-17)
- Test `tests/gates/test_setup-node-pnpm.bats` (Create)

**Step 1: Write the failing test**

```bash
cat > tests/gates/test_setup-node-pnpm.bats <<'BATS'
#!/usr/bin/env bats
...
```bash
bats tests/gates/test_setup-node-pnpm.bats
```
...
`bump.yaml`은 두 잡(`writeback` 51-54, `writeback-dispatch` 142-145) 모두 교체. `_audit.yaml`(14-17), `_teardown.yaml`(36-39), `bump-poll.yaml`(64-67), `onboard.yaml`(41-44), `_create-app.yaml`(57-60), `_create-database.yaml`(33-36), `_create-cache.yaml`(32-35) 동일. 각 워크플로에서 composite 채택 시 checkout(토큰 포함) step은 **앞에 그대로 유지**한다 — composite는 checkout 이후에 와야 한다(레포 컨텍스트 필요).

> dispatch-mutation.yaml은 `setup-node`만 쓰고 `pnpm install`을 안 하므로(검증 전용) **이 composite를 채택하지 않는다** — frozen-install이 불필요한 오버헤드. 테스트의 9개 목록에서 의도적으로 제외.

**Step 4: Run test, expect PASS**

```bash
bats tests/gates/test_setup-node-pnpm.bats tests/gates/test_workflow-yaml.bats
```
Expected: 전부 `ok`.
...
```bash
git -C /Users/ukyi/workspace/homelab-cicd-hardening add .github/actions/setup-node-pnpm/action.yml .github/workflows/ci.yaml .github/workflows/onboard.yaml .github/workflows/bump.yaml .github/workflows/bump-poll.yaml .github/workflows/_create-app.yaml .github/workflows/_create-database.yaml .github/workflows/_create-cache.yaml .github/workflows/_teardown.yaml .github/workflows/_audit.yaml tests/gates/test_setup-node-pnpm.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "refactor: setup-node-pnpm composite — node/pnpm 핀 SSOT + 9 워크플로 채택"
```
...
- Modify `.github/workflows/tf-reconcile.yaml` (reconcile init 60-66, drift-github init 142-148, drift-tailscale init 204-210)
- Test `tools/tests/test_tf-r2-init.bats` (Create)

**Step 1: Write the failing test**

```bash
cat > tools/tests/test_tf-r2-init.bats <<'BATS'
#!/usr/bin/env bats
...
```bash
bats tools/tests/test_tf-r2-init.bats
```
...
```bash
bats tools/tests/test_tf-r2-init.bats tests/gates/test_workflow-yaml.bats infra/_tests/test_tf_reconcile.bats
```
...
**Files:**
- Create `tests/gates/test_app-token-sha-ssot.bats`
- Delete `.github/actions/homelab-token/action.yml`, `tests/gates/test_homelab-token.bats`
- (SSOT 상수는 canonical SHA를 테스트가 직접 보유 — composite 삭제 후엔 가장 많이 쓰이는 인라인 핀이 권위)

**Step 1: Write the failing test**

```bash
cat > tests/gates/test_app-token-sha-ssot.bats <<'BATS'
#!/usr/bin/env bats
...
```bash
bats tests/gates/test_app-token-sha-ssot.bats
```
...
```bash
git -C /Users/ukyi/workspace/homelab-cicd-hardening rm .github/actions/homelab-token/action.yml tests/gates/test_homelab-token.bats
```
...
```yaml
      ...
      # (핀 SSOT: tests/gates/test_app-token-sha-ssot.bats가 전 콜사이트 동일 SHA 강제).
```
...
```bash
git -C /Users/ukyi/workspace/homelab-cicd-hardening add tests/gates/test_app-token-sha-ssot.bats .github/workflows/bump.yaml .github/workflows/onboard.yaml && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "refactor: app-token 핀 SSOT 단언 + 미사용 homelab-token composite 삭제"
```
...
`.apprepo`는 외부(비신뢰) 앱 레포를 sparse-checkout하는 경로(`_create-app.yaml:53`, `_update-secrets.yaml:52`)인데 `.gitignore`에 없다. `_teardown.yaml:64`는 `git add -A`라 만약 워크스페이스에 비신뢰 파일이 섞이면 통째 커밋될 위험이 있다. (1) `.apprepo/`를 gitignore에 추가하고, (2) teardown의 `git add -A`를 tool이 쓰는 명시 경로로 좁힌다.

**Files:**
- Modify `.gitignore` (외부 앱 체크아웃 경로 추가)
- Modify `.github/workflows/_teardown.yaml` (`git add -A` → 명시 경로)
- Test `tests/gates/test_apprepo-gitignore.bats` (Create)

**Step 1: Write the failing test**

```bash
cat > tests/gates/test_apprepo-gitignore.bats <<'BATS'
#!/usr/bin/env bats
...
```bash
bats tests/gates/test_apprepo-gitignore.bats
```
...
`_teardown.yaml:64`의 `git add -A`를 teardown 도구가 쓰는 트리로 한정. teardown-app은 `apps/<app>` + `docs/memory-ledger.md` + `infra/cloudflare/apps.json`을, teardown-resource는 `platform/`(cnpg/cache/data-conn) + 원장을 건드린다 — 둘을 포괄하되 `.apprepo`는 절대 포함하지 않는 명시 목록:
...
```bash
bats tests/gates/test_apprepo-gitignore.bats tools/tests/test_teardown.bats
```
...
```bash
git -C /Users/ukyi/workspace/homelab-cicd-hardening add .gitignore .github/workflows/_teardown.yaml tests/gates/test_apprepo-gitignore.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "fix: .apprepo gitignore + teardown git add -A를 명시 경로로 (비신뢰 체크아웃 격리)"
```
...
- Test (new) `tests/gates/test_ci-blocking-comment.bats`

**Step 1: Write the failing test**
```bash
cat > tests/gates/test_ci-blocking-comment.bats <<'EOF'
#!/usr/bin/env bats
...
**Step 2: Run it, expect FAIL**
```bash
bats tests/gates/test_ci-blocking-comment.bats
```
...
**Step 5: Commit**
```bash
git -C /Users/ukyi/workspace/homelab-cicd-hardening add .github/workflows/ci.yaml tests/gates/test_ci-blocking-comment.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "docs: ci.yaml audit 게이트 주석을 실제 BLOCKING 셋과 일치 + 드리프트 가드"
```

---

### Task 2: verify.yaml·Makefile이 pnpm verify:ledger SSOT 호출 (dry-9)

ledger 검증이 3곳에 다르게 철자돼 있다: `package.json:18`(`verify:ledger` = SSOT),
`verify.yaml:33-35`(인라인 `ledger-to-json.sh | conftest`), `Makefile:38-39`(동일 인라인).
`verify.yaml`과 `Makefile`이 `pnpm verify:ledger`를 호출하게 수렴한다(파이프라인 1곳 정의).

**주의:** `verify.yaml` verify 잡은 의도적으로 pnpm/node를 안 쓴다(주석 18-20: pnpm@11 self-install이
...
package.json·verify.yaml·Makefile 셋 다 그 스크립트를 호출 — pnpm 의존 도입 없이 3중 복제를 제거한다.

**Files:**
- Create `scripts/verify-ledger.sh` (SSOT 파이프라인)
- Modify `package.json:18` (`verify:ledger` → 스크립트 호출)
- Modify `.github/workflows/verify.yaml:32-35`
- Modify `Makefile:36-40` (`verify` 타겟)
- Test (new) `tests/gates/test_verify-ledger-ssot.bats`

**Step 1: Write the failing test**
```bash
cat > tests/gates/test_verify-ledger-ssot.bats <<'EOF'
#!/usr/bin/env bats
...
@test "verify.yaml ledger step no longer inlines the conftest pipeline" {
  # 인라인 'conftest test /tmp/ledger.json'이 verify.yaml에 남아있으면 실패(SSOT 미수렴).
  run grep -c 'conftest test /tmp/ledger.json' "$ROOT/.github/workflows/verify.yaml"
  [ "$output" = "0" ]
  grep -q 'scripts/verify-ledger.sh' "$ROOT/.github/workflows/verify.yaml"
}
...
**Step 2: Run it, expect FAIL**
```bash
bats tests/gates/test_verify-ledger-ssot.bats
```
...
# package.json(verify:ledger)·verify.yaml·Makefile(verify)·make ci가 모두 이 스크립트를 호출한다.
set -euo pipefail
...
`.github/workflows/verify.yaml:32-35` 교체:
```yaml
...
**Step 5: Commit**
```bash
git -C /Users/ukyi/workspace/homelab-cicd-hardening add scripts/verify-ledger.sh package.json .github/workflows/verify.yaml Makefile tests/gates/test_verify-ledger-ssot.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "refactor: ledger 검증을 scripts/verify-ledger.sh SSOT로 수렴 (인라인 conftest 3중 복제 제거)"
```
...
- Test (extend) `tools/tests/test_audit-orphans.bats` (active:false 행 비차단 케이스 추가)

**Step 1: Write the failing test** — `tools/tests/test_audit-orphans.bats`에 추가:
```bash
cat >> tools/tests/test_audit-orphans.bats <<'EOF'
...
**Step 5: Commit**
```bash
git -C /Users/ukyi/workspace/homelab-cicd-hardening add tools/audit-orphans.mjs tools/tests/test_audit-orphans.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "fix: orphan-dns 차단을 active:true 행으로만 한정 (active:false는 비차단 정보 — dns.tf와 정합)"
```
...
- Test (new) `tools/tests/test_ledger-totals.bats`

**Step 1: Write the failing test**
```bash
cat > tools/tests/test_ledger-totals.bats <<'EOF'
#!/usr/bin/env bats
...
**Step 2: Run it, expect FAIL**
```bash
bats tools/tests/test_ledger-totals.bats
```
...
bats tools/tests/test_create-app.bats tools/tests/test_teardown.bats tools/tests/test_provision-cache.bats tools/tests/test_onboard.bats
```
Expected: ledger-totals 3 `ok`; 기존 4 스위트 0 failures(Totals 프로즈가 정상 매치되므로 동등 출력).

**Step 5: Commit**
```bash
git -C /Users/ukyi/workspace/homelab-cicd-hardening add tools/lib/ledger-totals.mjs tools/teardown-app.mjs tools/create-app.mjs tools/provision-cache.mjs tools/onboard-app.mjs tools/tests/test_ledger-totals.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "refactor: ledger Totals 치환을 replaceTotals 헬퍼로 SSOT화 + 프로즈 드리프트 시 fail-loud"
```
...
- Test (extend) `tools/tests/test_poll-ghcr.bats` (transient → refuse 케이스)

**Step 1: Write the failing test** — `tools/tests/test_poll-ghcr.bats`에 추가:
```bash
cat >> tools/tests/test_poll-ghcr.bats <<'EOF'
...
**Step 5: Commit**
```bash
git -C /Users/ukyi/workspace/homelab-cicd-hardening add tools/poll-ghcr.mjs tools/tests/test_poll-ghcr.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "fix: poll-ghcr가 manifest 404와 transient 오류 구분 (transient는 rethrow→refuse fail-closed)"
```
...
- Test (new) `tools/tests/test_audit-dangling-role.bats`

**Step 1: Write the failing test**
```bash
cat > tools/tests/test_audit-dangling-role.bats <<'EOF'
#!/usr/bin/env bats
...
**Step 2: Run it, expect FAIL**
```bash
bats tools/tests/test_audit-dangling-role.bats
```
...
**Step 5: Commit**
```bash
git -C /Users/ukyi/workspace/homelab-cicd-hardening add tools/audit-orphans.mjs tools/tests/test_audit-dangling-role.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "feat: audit-orphans에 dangling-role 체크 추가 (purge 후 cluster.yaml 잔존 role 감지)"
```
...
- Test (new) `tests/gates/test_build-notify.bats`

**Step 1: Write the failing test**
```bash
cat > tests/gates/test_build-notify.bats <<'EOF'
#!/usr/bin/env bats
...
**Step 2: Run it, expect FAIL**
```bash
bats tests/gates/test_build-notify.bats
```
...
bats tests/gates/test_workflow-yaml.bats
```
Expected: build-notify 4 `ok`; workflow-yaml 스위트 0 failures(YAML 파싱 통과).

**Step 5: Commit**
```bash
git -C /Users/ukyi/workspace/homelab-cicd-hardening add .github/workflows/build.yaml tests/gates/test_build-notify.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "feat: build.yaml에 telegram-notify 추가 (이미지 빌드 결과 가시화, source=배포)"
```
...
`dispatch-mutation.yaml:143`의 `notify-failure`는 `if: failure()`만 켜져 있어, owner가 큐잉된
...
**Files:**
- Modify `.github/workflows/dispatch-mutation.yaml:143` (if 조건) + `:159` (status 매핑)
- Test (extend) `tests/gates/test_dispatcher.bats`

**Step 1: Write the failing test** — `tests/gates/test_dispatcher.bats`에 추가:
```bash
...
EOF
```
(참고 — ⚠️ #53: 현재 `tests/gates/test_dispatcher.bats` setup은 `F="$(cd "$BATS_TEST_DIRNAME/../.." && pwd)/.github/workflows/dispatch-mutation.yaml"`로 **`$F`만 정의**한다(`$ROOT` 아님). 위 신규 @test는 `$F`(= dispatch-mutation.yaml)를 쓴다. 다른 워크플로(_audit 등) 참조가 필요하면 `$(dirname "$F")/_audit.yaml` 형태로.)

**Step 2: Run it, expect FAIL**
```bash
bats tests/gates/test_dispatcher.bats
```
Expected: `not ok ... notify-failure fires on cancelled` — 현재 `if: failure()`라 `|| cancelled()`
정규식 grep이 비-0(매치 0).

**Step 3: Minimal implementation** — `.github/workflows/dispatch-mutation.yaml`:
...
**Step 5: Commit**
```bash
git -C /Users/ukyi/workspace/homelab-cicd-hardening add .github/workflows/dispatch-mutation.yaml tests/gates/test_dispatcher.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "fix: dispatch-mutation notify-failure를 failure() || cancelled()로 + 취소 상태 매핑"
```

---

### Task 9: _audit.yaml telegram body의 [:20] 캡·에러삼킴 제거 (obs-6)

`_audit.yaml:27`은 audit 결과를 `.findings[:20]`로 잘라 (20건 초과 드리프트를 숨김) +
...
**Files:**
- Modify `.github/workflows/_audit.yaml:27` (jq 표현식)
- Test (new): 기존 `tests/gates/test_workflow-yaml.bats`나 신규 grep 단언 — 여기선 `_audit.yaml` 전용
  정적 단언을 `tests/gates/test_build-notify.bats`에 묶지 않고 별도로 `dispatcher.bats`에 추가(이미 audit
  잡 라우팅을 검증하는 스위트). 단순·격리 위해 `tools/tests/test_audit-orphans.bats`가 아닌 워크플로
...
**Step 3: Minimal implementation** — `.github/workflows/_audit.yaml:23-32` (build audit summary 스텝) 교체:
```yaml
...
**Step 5: Commit**
```bash
git -C /Users/ukyi/workspace/homelab-cicd-hardening add .github/workflows/_audit.yaml tests/gates/test_dispatcher.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "fix: _audit.yaml 알림에서 findings [:20] 캡·jq 에러삼킴 제거 (전 드리프트 가시화)"
```
...
```bash
bats tests/gates/test_ci-blocking-comment.bats tests/gates/test_verify-ledger-ssot.bats \
     tools/tests/test_ledger-totals.bats tools/tests/test_audit-orphans.bats \
     tools/tests/test_audit-dangling-role.bats tools/tests/test_poll-ghcr.bats \
     tests/gates/test_build-notify.bats tests/gates/test_dispatcher.bats
...
> **컨텍스트 (read-before-edit로 확인):** `platform/victoria-stack/prod/deadmanswitch-relay.yaml`은 ConfigMap(`relay.sh`) + Deployment + Service. Alertmanager의 항상-발화 Watchdog가 `http://deadmanswitch-relay:9095/ping`(`platform/victoria-stack/prod/alertmanager.yaml:79`)로 webhook을 보내고, 릴레이는 그걸 받을 때 healthchecks.io로 ping을 전달한다. healthchecks.io에서 **ping의 부재**가 곧 page다 — 즉 "webhook 미수신인데도 healthchecks가 green"이면 dead-man switch가 무력화된다.
>
...
>
> **게이트 배선(#53 모델):** `platform/victoria-stack/prod/test_relay.bats`는 이미 존재하며(현재 `-q` 가드 1건 — ⚠️ setup depth 버그로 false-green, Step 1에서 함께 수정), 게이트 수집 SSOT `scripts/run-bats.sh`가 자동 포함한다(git-tracked `test_*.bats` − `platform/charts/*` − `tests/.ci-exclude`; charts 밖 + .ci-exclude 미등재라 포함 — `./scripts/run-bats.sh --list`로 확인). cnpg `test_creds_reference`/`test_drill_alerting`/`test_kustomize_build`는 이제 ci.yaml 인라인 글롭이 아니라 `tests/.ci-exclude`에 있다. 신규 파일을 만들지 않고 이 파일을 확장한다.
...
**Files:**
- Modify `platform/victoria-stack/prod/deadmanswitch-relay.yaml`:5-16 (ConfigMap `relay.sh`의 while 루프)
- Test `platform/victoria-stack/prod/test_relay.bats` (기존 파일 확장 — gate 글롭 자동 포함)
...
`platform/victoria-stack/prod/test_relay.bats`를 아래로 교체한다(기존 setup/첫 @test 유지 + 신규 3건):
...
```bash
bats platform/victoria-stack/prod/test_relay.bats
```
...
`platform/victoria-stack/prod/deadmanswitch-relay.yaml`의 ConfigMap `relay.sh`(5-16행)를 교체한다. `nc`를 `if`의 조건으로 두면 `set -eu`가 루프를 죽이지 않는다(compound 조건 실패는 `set -e` 비대상 — 단, 중간 `[[ ]]` 함정은 bats 한정이라 여기 sh 스크립트엔 무관). `wget`은 nc 성공 분기 안으로 중첩, 실패 분기엔 `sleep 5`.
...
yq 'select(.kind=="ConfigMap").data."relay.sh"' platform/victoria-stack/prod/deadmanswitch-relay.yaml | sha256sum | cut -c1-16
```
...
게이트 통합 확인(run-bats.sh 수집 SSOT — #53 모델):

```bash
./scripts/run-bats.sh --list | grep -q 'platform/victoria-stack/prod/test_relay.bats' && echo "수집 포함 OK"
bats platform/victoria-stack/prod/test_relay.bats   # 5 tests pass
```
기대: `수집 포함 OK` + `5 tests, 0 failures`. (별도 hardcoded find 글롭 불필요 — `tests/gates/test_run-bats.bats`가 수집 불변식을 강제.)

**Step 5: Commit**

```bash
git -C /Users/ukyi/workspace/homelab-cicd-hardening add platform/victoria-stack/prod/deadmanswitch-relay.yaml platform/victoria-stack/prod/test_relay.bats && git -C /Users/ukyi/workspace/homelab-cicd-hardening commit -m "fix: deadmanswitch relay nc-서빙 성공 시에만 healthchecks ping + checksum annotation으로 ArgoCD 자동 롤 (fm-1, F7)"
```
...
- **security-supplychain-1** (분기보호 불변식이 required gate 밖) — Accept: 테스트를 `tests/gates/test_branch_protection.bats`(gate 글롭)로 이동(이후 pass1 F1 표기와 동일 이슈를 pass1에서 교정).
...
- **drift-2 live-DNS documented-away** — Accept: 커밋된 opt-in `tools/dns-drift-check.mjs` + `dns-drift.yaml` 스케줄 워크플로.
