...
- 검출(perl, `-CSDA` UTF-8) — **검증 완료**:
  ```bash
  perl -CSDA -ne 'print "$ARGV:$.: $_" if /^\s*\@test\s+".*[\x{1100}-\x{11FF}\x{3040}-\x{30FF}\x{3130}-\x{318F}\x{4E00}-\x{9FFF}\x{AC00}-\x{D7A3}\x{FF00}-\x{FFEF}]/' "$f"
  ```
  - `^\s*\@test\s+"` 선언만(주석의 `# (@test 이름은 영어…)` 언급 제외).
  - CJK 범위만: Jamo(1100-11FF)·Kana(3040-30FF)·compat Jamo(3130-318F)·CJK 한자(4E00-9FFF)·Hangul 음절(AC00-D7A3)·반각/전각(FF00-FFEF). **em-dash(—, U+2014)·Latin 악센트는 제외**(메모리: em-dash OK, 한글만 깨짐).
