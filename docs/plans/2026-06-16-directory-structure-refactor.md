...
### 후속 재검증 라운드 (2026-06-16, 별도 세션 — hardened-planning Phase C 재진입, branch mode `--base 540b257`)

이전 finalize가 `needs-attention`(미검증 P5 반영)으로 끝난 점을 사용자 지시로 재검증. branch mode 2-pass로
**이전 5-pass가 놓친 W6/W7 결함 2건**을 추가 포착·반영하고 `approve`에 도달했다(아래가 위 "재리뷰하지 않음"을 대체).

| Pass | Finding | Sev | Decision | 반영 |
|---|---|---|---|---|
| R1 | W6 stale `.yml` 가드 `! git ls-files '.github/workflows/*.yml'`가 **항상 FAIL** (`git ls-files`는 매치 0에도 exit 0 → `!`가 출력 아닌 exit status를 부정) | H | Accept | 735·798·799를 `[ -z "$(git ls-files …)" ]`(출력-비어있음) idiom으로 통일. (역설: pass 3이 추가한 그 가드 자체가 broken이었음) |
| R1 | W7이 W6와 **병렬 선언**됐으나 W6가 rename하는 `verify.yml`을 편집 → PR-first auto-merge에서 rename+edit 충돌 | M | Accept | **W7→W6 순서 명시**(시퀀싱·preamble·W6/W7 헤더). W6 git-grep 소비자 스윕이 W7 잔여 `verify.yml` 참조 흡수 |
| R2 | (없음 — 0 findings) | — | — | **verdict `approve`** |

**R1 verdict:** `needs-attention` — *summary:* "the plan contains an impossible W6 gate and an unhandled W6/W7
ordering conflict." → 2건 반영.
**R2 verdict:** `approve` (0 findings) — *summary:* "No blocking plan defect found. The plan explicitly addresses …
collection loss, path consumers, ordering dependencies, render parity, owner-only live checks, rollback isolation by
PR, and final accounting gates." 반영분이 새 위험을 만들지 않음을 확인. **플랜이 codex `approve`에 도달**(이전
needs-attention finalize 대체). 본 라운드 dispositions도 사후 감사로 재리뷰 대상 아님.

---

## Execution directives

> 실행 세션이 어떻게 돌릴지 상속하는 블록(사후-승인 — 재리뷰 대상 아님). 위 "규약·안전 프로토콜"을 보강한다.

- **워크트리:** 이 플랜은 `refactor/dirstructure-plan` 워크트리(`~/workspace/homelab-dirstructure-plan`)에서 실행.
- **스킬:** **별도 세션**에서 `executing-plans`로 task-by-task 구현. 이 세션에서는 구현하지 않는다.
- **연속 실행:** 배치 사이에 일상 리뷰로 멈추지 않는다. **진짜 블로커**에서만 정지 — 의존성 부재, 반복 실패하는
  검증, 모호·모순 지시, 치명적 플랜 갭(executing-plans의 "When to Stop and Ask"). 그 외엔 전 배치를 끝까지 진행.
- **`[OWNER]` 마커 태스크**(라이브 `KUBECONFIG`·age 키 필요)는 에이전트가 준비만 하고 OWNER가 로컬 실행.
- **커밋 — 규칙을 직접 적용, `Skill(commit)` 호출 금지**(인터랙티브 확인이 연속 실행을 끊는다):
  - **언어:** 한국어 메시지. **AI 마커 금지** — `🤖 Generated with`·`Co-Authored-By: Claude` 등 절대 포함 금지.
  - **형식:** `<type>(<scope>): 한국어 설명` (필요 시 `- 상세` 본문 줄).
  - **type — 다음만 사용:** `feat`(새 기능)·`fix`(버그)·`refactor`(리팩토링/성능)·`docs`(문서)·`style`(포맷)·
    `test`(테스트)·`chore`(빌드/설정). `perf`/`build`/`ci` 등 사용 금지.
  - **그룹화(우선순위):** ① 같은 기능/모듈 디렉토리는 함께 ② 목적별 분리(refactor vs fix vs feature)
    ③ 서로 import/참조하는 파일은 함께 ④ 변경 종류로 분할 — config·테스트·문서·독립 style은 각각 별도 커밋.
  - **판단:** 같은 디렉토리+같은 목적 → 1커밋; 다른 파일 없이는 무의미한 변경 → 같은 커밋; 독립 설명 가능한
    변경 → 별도 커밋.
  - **위치:** 각 플랜 `Commit` 스텝에서 현재 feature 브랜치 워크트리에 직접 커밋(이미 `main`이 아니므로 새 브랜치 불필요).
- **PR 경계:** 각 워크스트림 = 1 PR(PR-first + auto-merge). 순서 의존 **W0→W1→W2**, **W7→W6**, 그 외 W1 후 병렬.
