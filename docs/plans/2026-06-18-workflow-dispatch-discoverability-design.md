...
- **teardown은 standalone 툴**: `_teardown.yaml`은 `node tools/teardown-app.mjs` /
  `teardown-resource.mjs`를 감싸 PR만 연다. 안전 로직(`.bindings.json` 참조 0 강제·retain/purge·플랜)은
  툴에 있어 **owner가 로컬에서 같은 툴 실행 → 본인 권한 PR → 사람 머지**로 동일 보장.
...
`teardown-app` · `teardown-resource` · `activate-app`을 Actions 노출에서 제외:
- `_teardown.yaml` 삭제(`tools/teardown-app.mjs`·`teardown-resource.mjs`는 유지).
- `activate-app`은 원래 클러스터 게이트라 디스패처에서 echo 스텁이었음 — 스텁째 제거.
- README/런북에 로컬 절차(`node tools/teardown-app.mjs --app <x>` 등)와 "왜 CI에 없는지"(파괴/로컬) 명시.
...
| 신설 | `create-app.yaml` · `update-secrets.yaml` · `create-database.yaml` · `create-cache.yaml`(전용 디스패처) · `audit.yaml`(스케줄) · `README.md` · 공유 실패-알림 단위(composite 추정) |
| 수정 | `build`·`bump-poll`·`tf-reconcile`·`pr-sweeper`·`dns-drift`·`renovate`(run-name 추가) · `AGENTS.md` · 직렬화 가드 테스트 |
