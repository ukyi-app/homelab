...
- `sourceRepos` = homelab repo + 각 매니페스트의 helm repoURL (라이브 Application에서 **파생**)
- `destinations` = 현 platform Application들의 destination ns (**파생**) + in-cluster server
- `clusterResourceWhitelist: ['*']` — 플랫폼은 CRD/ClusterRole/GatewayClass/Namespace를 정당하게
  설치하므로 좁히면 동작파괴. 와일드카드 유지(가치=server+repo+ns 경계).

**파생값 안전장치**: sourceRepos/destinations는 추측하지 않고 라이브 Application 집합에서
enumerate하며, "모든 platform Application의 repoURL ∈ sourceRepos ∧ destination.namespace ∈
destinations"를 **bats 가드**로 강제(드리프트 차단). 누락 시 sync 거부(동작파괴)를 커밋 전에 막는다.
...
2. `root/apps/namespaces.yaml` 수동 Application 추가(project=platform, sync-wave -9, 자체
   resources-finalizer, SSA, destination=in-cluster server) → 동일 매니페스트라 기존 네임스페이스를
   **adopt**(OutOfSync 0).
...
- platform 파생-가드 bats: 모든 platform Application repoURL ∈ sourceRepos, destination ∈ destinations.
...
- 롤백: project 되돌림 1줄 / finalizer 제거 / 수동 namespaces app 되돌림 — 각 머지 독립 가역.
