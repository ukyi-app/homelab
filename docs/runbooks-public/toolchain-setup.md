...
신규 체크아웃에서 로컬 검증(`just ci`/`just verify`/`just chart-test`)을 돌리는 데 필요한
...
>
> 아래 표가 소유하는 것은 **로컬 최소 버전**(`justfile`의 `m6-tools` 타겟 — 설치 후
> `just m6-tools`로 검증) + 설치 힌트뿐이다. 로컬 최소는 **하한**이고 CI 핀은 **정확 핀**이라
...
| **just** | 1.58.0 | 레포 명령 실행 | `mise use -g just@1.58.0` 또는 `brew install just` |
| **helm** | `>=3.16` | 공유 차트 렌더(chart-test) | `brew install helm` (CI는 고정 핀 — major 변동 시 chart-test 파손 위험) |
| **kustomize** | (게이트 없음) | KSOPS 풀 렌더(`just render`) | `brew install kustomize` |
...
| **shellcheck** | (게이트 없음) | `*.sh` 린트(`just ci`) | `brew install shellcheck` (버전이 다르면 info 체크가 CI와 드리프트) |
...
- **terraform** — `just tf-validate`/IaC 루트용. owner와 CI 실행 핀은 함께 갱신한다.
...
- **actionlint** — 워크플로 정적 검사. **CI 전용**이라 `m6-tools` 필수가 아니다. 로컬에 없으면
  `just ci`가 그 스텝을 미평가 원장에 남기고 마지막에 `SKIP:` 마커 + exit 4를 낸다(초록으로 끝나지 않는다).
- **docker** — telegram-render-e2e 게이트·로컬 dev Postgres(`bun run db:up`). 없으면 위와 같은 SKIP 경로.
- **kubectl** — 라이브 클러스터 운영 타겟(`just argo-*`/`render`/posture). 클러스터 minor와 ±1 권장.
- **pre-commit** — 평문 시크릿 가드 + gitleaks(`pre-commit run -a`). `brew install pre-commit` 후 `pre-commit install`.

## 설치 후 검증

```bash
just m6-tools        # helm/kubeconform/bats/bun(1.4.2)/yq/jq 최소 버전 게이트
...
just ci              # required check(gate)를 로컬 재현 — 차이는 policy/ci-parity.json이 계상
```

`just`/git hook/Claude Bash는 셸 rc를 source하지 않는다 — **레포는 PATH를 손대지 않으므로**
