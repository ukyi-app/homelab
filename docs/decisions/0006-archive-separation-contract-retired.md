...
- ~~`s3://homelab-pg-backups-prod/pg/`(Mac 아카이브)는 **삭제되지 않는다.** PONR 1은 별도 결정으로
  ...
  🔴 **정정(2026-09-03, 감사 3라운드) — 이 문장은 쓰여진 시점에 이미 거짓이었다.**
  "삭제되지 않는다"의 근거로 든 것은 **사람의 삭제 결정(PONR 1)이 미뤄졌다**는 사실인데, 이 버킷에는
  기계적 삭제자가 따로 있다: `infra/cloudflare/r2.tf`의 `cloudflare_r2_bucket_lifecycle.pg_backups`가
  `conditions = { prefix = "" }` + `Age 1209600`(14일)로 **버킷 전체**를 만료시킨다. 이 라이프사이클은
  fe28e9b(이 ADR보다 앞)부터 있었고 Mac 아카이브의 쓰기는 2026-08-13에 얼었으므로, `pg/` 객체는
