...
명목 잔여(9216−8440 = 776)는 실 헤드룸을 과소표현한다 — 앱이 0개여도 잔여가 작아 *보이는* 이유다.
더 많은 명목 헤드룸이 필요하면 (a) 위 보수 버퍼 행을 라이브 실측에 맞게 축소해 limit 예산을 회수한다
(cert-manager·edge·cnpg; 단 cnpg ~200Mi는 WAL/백업 burst 마진이라 KUBECONFIG 실측 후에만 손댄다).
...
**합계:** req ≈ 4467 Mi · limit ≈ 8440 Mi (반드시 ≤ 9216 Mi 유지).
