...
limit over-commit을 허용하므로, limit 합(현재 7632)은 *동시-peak 상한*일 뿐 실사용이 아니다.
...
pod만 ~170Mi)·`cert-manager`(unlimited operator reserve). (`edge`·`cnpg`의 보수 버퍼는 2026-06-22
right-size에서 라이브에 정합 회수.) 따라서 명목 잔여(9216−7632 = 1584)는 실 헤드룸을 과소표현한다.
더 많은 명목 헤드룸이 필요하면 (a) 상주 워크로드를 라이브 peak 실측에 맞게 right-size해 limit을 회수한다
(2026-06-22 observability/argocd/edge/cnpg 808Mi 회수; postgres·cert-manager·최근 OOM 수정분은 보호).
...
| <!-- ledger:row --> argocd         | argocd         |    576 |     1344 |
| <!-- ledger:row --> cnpg           | database       |    900 |     1152 |
| <!-- ledger:row --> cert-manager   | cert-manager   |     60 |      180 |
| <!-- ledger:row --> observability  | observability  |   1120 |     2144 |
| <!-- ledger:row --> edge           | edge           |    240 |      416 |
...
**합계:** req ≈ 4275 Mi · limit ≈ 7632 Mi (반드시 ≤ 9216 Mi 유지).
