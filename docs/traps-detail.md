...
- **처방(원인 측)**: `--apply`는 reconfigure 뒤 `net_settled`가 참이 될 때까지 1초 간격으로 기다린 뒤에야
  ...
  루프를 통째로 빼면 둘 다 red. ⚠️ 반복 횟수·문자열만으로는 `sleep 1`을 `:`로 바꾼 busy-loop가 35/35 초록이었다
  (13라운드 reg13-b2-domain-tests-1) — 경과 시간 하한(`date +%s` 전후 차 ≥ 폴링 간격 합)을 얹으면 busy-loop는
  둘 다 red지만, 초 단위 스냅샷 차는 **절반 대기**(`sleep 0.5`)도 초 경계 반올림으로 15회 중 4회 통과시킨다
  (13c reg13c-a-landing-hunks-3). 결정적 증인은 `sleep` 시임이 기록한 **호출 수·인자 등식**(2회 · 전부 1)이고
  경과 시간 하한은 보조로 남긴다 — `date +%s%N`은 macOS에 없어 쓰지 않는다).
