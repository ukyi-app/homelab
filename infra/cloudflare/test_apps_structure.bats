...
@test "apps.json hosts do not collide with reserved names (apex/www/home suffix, case-normalized)" {
  # DNS는 대소문자를 구별하지 않는데 아래 술어는 구별한다 — WWW./Files./x.Home. 같은
  # case variant 3종이 5/5 초록으로 통과했다(실측). 정규화 대신 표기 자체를 계약으로 고정한다
...
@test "reserved-hosts.json is valid and lists fully-qualified platform hosts" {
  # 거울 구멍 — 예약 목록 쪽이 대문자면 위 :22 표기 계약과 어긋난 채로 통과한다.
...
@test "reserved-hosts.json pins the exact web-public platform host set" {
  # length>0은 바닥값이라 원소 삭제만 잡는다 — 원소 추가·교체(공개 표면 SSOT 자체의
  # 신원 변조)는 상한이 없어 19/19 초록이었다(실측, files.ukyi.app→evil.ukyi.app 포함).
