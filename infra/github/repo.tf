...
resource "github_branch_protection" "main" {
  ...
  # admin 강제를 켜면 owner 직접 머지 경로가 막혀 운영 불가.
  # ⚠️ enforce_admins=false는 required status check(gate)와 strict까지 **admin에게 면제**한다 —
  #    잔여 우회는 owner의 수동 머지 경로 전부다(라이브 확인 2026-09-03: contexts=["gate"]·strict=true·
  #    enforce_admins=false). 자동화 주체(writer App)는 admin이 아니고 auto-merge도 required check를
  #    우회하지 않으므로 gate에 그대로 걸린다. residual bypass임을 branch_protection.bats가 잠근다.
