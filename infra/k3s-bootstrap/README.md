...
**AIOps는 별도 설치** — `bash infra/k3s-bootstrap/aiops-install.sh --prepare`로 설치 계획을 생성한다.
...
- `host-config/` — 그대로 `/`에 놓이는 드롭인 트리(resolved · journald · sshd · systemd-networkd 드롭인 · tmpfiles.d + systemd 유닛: files-data-backup·systemd-failed-sweep 타이머 쌍과 unit-failure-notify@). `diff`가 곧 리뷰이고 **트리 열거가 곧 검사 도메인**이다.
...
경로는 두 국면에 걸쳐 불변이다 — 그래서 전환에서 PV hostPath 재작성도, platform 매니페스트 재수정도 없었다. **국면 B는 2026-08-26에 진입 완료**(2TB M.2 `/dev/nvme1n1p1` → `/mnt/bulk`, `BULK_MIGRATION_WINDOW_UNTIL=""`, r4 `FilesBackupStale`의 한시 억제 절 제거 동반). 아래는 그 전환 순서의 기록이자 **국면 A를 다시 열 때 밟을 절차**다:
