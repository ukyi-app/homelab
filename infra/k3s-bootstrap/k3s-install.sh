...
# versions.env의 K3S_KUBECONFIG_NAME으로 각인한다(D-i). 시임이 아니라 **핀**이다 — 컷오버 시기
# 옛 클러스터 kubeconfig와 경로·포트·노드명이 전부 같아서, 이 이름이 두 클러스터를 가른 첫 텍스트 단서였다.
#
...
# 바꾼다. 컷오버 시기 옛 클러스터 kubeconfig와 경로·포트·노드명이 전부 같아서, 이 이름이 두
# 클러스터를 가른 첫 텍스트 단서였다. 위임하는 이유: 소급 적용(이미 회수된 파일)이 **같은 구현**을
# 써야 하기 때문이다.
