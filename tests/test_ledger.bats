...
@test "seed ledger passes the budget policy" {
  bun tools/ledger-to-json.ts docs/memory-ledger.md > "$BATS_TEST_TMPDIR"/ledger.json
  run conftest test "$BATS_TEST_TMPDIR"/ledger.json --policy policy/ledger.rego
  [ "$status" -eq 0 ]
}
...
@test "a ledger whose row markers drifted is rejected by the scan-floor, not silently vacuous" {
  sed 's/ledger:row/ledger:ROW/g' docs/memory-ledger.md > "$BATS_TEST_TMPDIR"/drift-ledger.md
  bun tools/ledger-to-json.ts "$BATS_TEST_TMPDIR"/drift-ledger.md > "$BATS_TEST_TMPDIR"/drift.json
  run conftest test "$BATS_TEST_TMPDIR"/drift.json --policy policy/ledger.rego
  [ "$status" -ne 0 ]
...
@test "a partially parsed ledger is rejected too (drift need not be total)" {
  awk '/ledger:row/ { n++; if (n > 8) next } { print }' docs/memory-ledger.md > "$BATS_TEST_TMPDIR"/partial-ledger.md
  bun tools/ledger-to-json.ts "$BATS_TEST_TMPDIR"/partial-ledger.md > "$BATS_TEST_TMPDIR"/partial.json
  run conftest test "$BATS_TEST_TMPDIR"/partial.json --policy policy/ledger.rego
  [ "$status" -ne 0 ]
  echo "$output" | grep -q 'scan-floor'
}
...
@test "a single row falling out of the row class is rejected (partial drift under-sums the budget)" {
  sed -E 's/(ledger:row --> )observability/\1X_observability/' docs/memory-ledger.md > "$BATS_TEST_TMPDIR"/one-row.md
  run bun tools/ledger-to-json.ts "$BATS_TEST_TMPDIR"/one-row.md
  [ "$status" -ne 0 ]
