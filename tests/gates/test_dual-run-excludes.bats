...
@test "no apps/ path is excluded from the platform appset (app exclusion is never the dual-run lever)" {
  # ⚠️ 인-레포 배포 앱은 page 1개다(apps/README.md가 SSOT) — 이 단언은
  #    이제 공허하지 않다. 병행 운용 중이던 시절 "다른 클러스터와 겹치니 앱을 빼두자"가 재발하는
  #    것을 막는 자리다. 분리는 cloudflared(공개 인입)와 serverName(아카이브)으로 하지, 앱
  #    Application을 지우는 방식으로 하지 않는다.
