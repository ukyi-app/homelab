...
@test "every age budget also clears the GitHub scheduler lag ceiling (declared cron is not the real period)" {
  ...
  #
  #   ── 2026-09-02 재측정: 위 21600s는 **이미 지나간 체제의 숫자**다. 두 체제를 분리해 적는다
  #      (한 숫자에 섞으면 「측정 창이 기판 변경을 가로지르면 두 체제가 한 숫자에 섞인다」를 그대로 밟는다).
  #      경계는 2026-08-26T17:00Z — 단주기 3종이 **동시에** 굶기 시작한 시점이고 원인은 GitHub 측이다.
  #        [이전] bump-poll p50 31m/MAX 112m · tf-reconcile p50 35m/MAX 100m · pr-sweeper p50 35m/MAX 95m
  #               — 세 워크플로 모두 6h 초과 0건. 일 run 수 38~53건.
  #        [이후] bump-poll p50 265m/MAX 750m(45011s) · tf-reconcile p50 238m/MAX 717m · pr-sweeper p50 239m/MAX 713m
  #               — 6h 초과 7~8건/워크플로. 일 run 수 2~7건. `ALERTS{alertname="GHAWorkflowStale"}`
  #               21 에피소드·약 3440분(08-27T08:50Z~08-31T17:20Z) — **전부 정상 동작 중인 워크플로**였다.
  #               bats:124의 2026-07-30 라이브 실패와 같은 클래스가 천장 위에서 재현된 것이다.
  #      ★ 처방의 실제 근거는 스파이크 MAX(45011s)가 **아니다** — 그건 참고값이다. 스파이크가 가라앉은
  #        가장 잔잔한 44h(08-31T18:00Z~)만 다시 재도 MAX가 333m/354m/343m로, 21600s까지 여유가
  #        각각 **27분/6분/17분**뿐이다. 분포 자체가 이동해 천장이 정상 동작의 꼬리 안에 앉았다
  #        (원설계 ×1.5 → 실효 ×1.02~1.08). 임계가 꼬리 안에 있으면 재발은 확률이 아니라 시간 문제다.
  #      ⇒ 천장을 **86400s(1일)** 로 올린다. 측정에 맞추지 않고 **알림의 목적에서 유도한** 값이다 —
  #        이 알림이 잡는 것은 영구 정지(60일 자동 비활성화·Actions 비활성화·스케줄러 유실)이고 그건
  #        시간 단위가 아니라 일 단위 사건이다. 감지 지연 1일은 목적에 충분하다(즉시성은 AGENTS.md가
  #        못박은 대로 deploy-trigger dispatch가 지고, tf-reconcile의 1차 경로는 iac.yaml push-apply다).
  #        ×1.x 마진으로 스케줄러 분포를 **쫓는 방식 자체**가 5주에 세 번의 재보정을 낳았다(07-30 →
  #        08-22~26 → 08-27~). 86400은 exporter의 dns-drift.yaml 예산과 같은 값이라 WORKFLOWS 블록에
  #        새 매직넘버가 늘지도 않는다.
  ...
  [ "${LAG_CEILING:-0}" -ge 86400 ] || { echo "LAG_CEILING이 소실/축소됐다: '${LAG_CEILING:-<unset>}'"; false; }
