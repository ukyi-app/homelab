#!/usr/bin/env bats
# 운영 just 진입점 — AGENTS.md 산문에만 있던 argo patch/kustomize 풀렌더를 타겟화.
# dry-run(just --dry-run)으로 명령 구성만 검사(라이브 클러스터 불필요). read-only 전제.
# ⚠️ 중간 단언은 [ ]만 — bash 3.2 [[ ]] 침묵 통과.

setup() { ROOT="$(cd "$BATS_TEST_DIRNAME/../.." && pwd)"; cd "$ROOT" || exit 1; }

@test "argo-sync composes an explicit-sync patch for the given APP" {
  run just --dry-run APP=cnpg argo-sync
  [ "$status" -eq 0 ]
...
@test "argo and render targets point at the live kubeconfig / KSOPS flags" {
  ...
  run just --dry-run COMP=cnpg render
  [ "$status" -eq 0 ]
...
@test "just verify-posture target exists and is live-guarded" {
  run grep -E '^verify-posture:' justfile
  [ "$status" -eq 0 ]
  # 원문 grep이 아니라 `just --dry-run` — 대상 스위트가 POSTURE_BATS 시임으로 빠져 원문엔 리터럴이 없다.
  # 해소된 recipe를 보는 쪽이 실제 행동에 더 가깝다.
  run just --dry-run verify-posture
  [ "$status" -eq 0 ]
...
# ⚠️ prerequisite가 아니라 recipe 줄이어야 한다(test_guard-skip-signalling이 just verify-posture를
#    실제로 실행하므로 prerequisite는 그 @test들을 죽인다). just --dry-run 이 recipe 줄을 **순서대로**
...
@test "mutating ops targets assert cluster identity before touching the cluster" {
  ...
  # 변이 명령 **뒤**로 밀려도 초록이다(실측: justfile argo-sync의 `@$(ASSERT_IDENTITY)`와
  ...
  for spec in "argo-sync:kubectl -n argocd patch app" \
    ...
    # 배치 — `just --dry-run`이 recipe 줄을 순서대로 보여주므로 줄 번호로 고정한다(관용구: test_pr-sweeper.bats).
...
@test "environment inputs remain supported and explicit overrides take precedence" {
  run env APP=from-env just --dry-run argo-sync
  [ "$status" -eq 0 ]
  printf '%s\n' "$output" | grep -q 'patch app from-env'
  run env APP=from-env just --dry-run APP=from-cli argo-sync
  [ "$status" -eq 0 ]
  printf '%s\n' "$output" | grep -q 'patch app from-cli'
}

@test "invocation from a subdirectory keeps the repository kubeconfig path" {
  cd tools
  run just kubeconfig
  [ "$status" -eq 0 ]
  [ "$output" = "export KUBECONFIG=$ROOT/infra/k3s-bootstrap/kubeconfig" ]
}
