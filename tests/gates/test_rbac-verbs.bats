#!/usr/bin/env bats
# platform RBAC verb 화이트리스트 게이트 — 손으로 쓴 ClusterRole/Role **전부**의 verb가
# read-only 집합 안에 있는지. 쓰기 verb는 파일 단위 면제 로스터로만 산다. @test 이름은 영어.
#
...
#    착지한다」). 165개 매니페스트 전수 yq가 0.6초라 좁힐 이유도 없다(실측).
...
setup() {
  ...
  # 파일 열거 = `tools/lib/repo-walk.ts`의 `platform-manifests` 스코프와 같은 어휘다
  # (추적 platform yaml − 벤더 `charts/`·`barman-plugin/`·gateway-api CRD 번들 − 테스트 하네스).
  # 사본이 아니라 어휘 일치다 — bats는 bun을 부르지 않는 CI-safe 정적 가드로 유지한다.
  MANIFESTS="$(git ls-files -- platform \
    | grep -E '\.ya?ml$' \
  ...
  # 붕괴 경계 120(현재 165): 컴포넌트를 정당하게 철거할 여유를 두되 붕괴는 못 넘긴다.
  [ "$NMANIFEST" -ge 120 ] || { echo "platform 매니페스트 열거가 ${NMANIFEST}건으로 붕괴했다(기대 >=120)"; false; }
  local f
  RBACFILES=""
  ...
  # 붕괴 경계 6(현재 9): 도메인 크기를 굳히면 컴포넌트를 철거할 때마다 red가 난다.
...
@test "the platform RBAC enumeration does not collapse (scan floor)" {
