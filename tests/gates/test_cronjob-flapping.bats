...
#
# ⚠️ 감시 목록을 **하드코딩과 대조하지 않는다** — 하드코딩 목록은 자기 자신에 대해서만 정확하다.
#    룰의 임계와 각 CronJob의 limit을 **레포에서 계산해** 대조하므로, 새 CronJob이 생기거나 임계가
#    바뀌면 자동으로 red가 된다.
...
EOF
  ...
  # (초판 `-ge 5`가 2건 스킵을 통과시켰다 — 뮤테이션 M3), 같은 파서로 세면 양쪽이 함께 깨진다.
