...
@test "no stale .yml references to renamed workflows (code·docs·tests·manifests)" {
  ...
  run bash -c "git -C '$ROOT' grep -lE '(_create-app|_create-cache|_create-database|_update-secrets|bump-poll|renovate|tf-reconcile|verify)\.yml' -- || true"
  [ -z "$output" ]
}

@test "deleted dispatch workflows have no tracked references" {
  # dispatch-mutation 멀티플렉서 + _audit/_teardown reusable 삭제 후 잔존 참조 0.
  # 제외: 외부계약 reusable-app-build·이 가드 파일 자신(패턴 정의가 자기매치).
  run bash -c "git -C \"$ROOT\" grep -lE 'dispatch-mutation|_audit\.yaml|_teardown\.yaml' -- ':!.github/workflows/reusable-app-build.yaml' ':!tests/gates/test_workflow-yaml.bats' || true"
  [ -z "$output" ]
}
...
@test "deleted v1 onboarding identifiers have no tracked references" {
  ...
  # 제외: docs/runbooks(로컬 런북, 별도 수동)·자기 가드 파일.
  run bash -c "git -C \"$ROOT\" grep -lE 'onboard\.yaml|onboard-app|homelab-app-schema|\.homelab\.yaml' -- ':!docs/runbooks/*' ':!tests/gates/test_workflow-yaml.bats' || true"
  [ -z "$output" ]
}
