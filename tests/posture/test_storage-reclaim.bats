#!/usr/bin/env bats
# files 데이터 내구성 posture: bulk-ssd SC와 files-data PV가 둘 다 라이브에서 Retain인지.
# git 승격(B5.1)이 라이브에 실제 반영됐고 인플레이스 마이그레이션(SC delete+recreate)이 완료됐는지 확인한다.
# LIVE: KUBECONFIG = files-prod가 sync된 k3s 노드 필요. @test 이름은 영어.
...
@test "the bound files-data PV carries Retain (existing PV migrated, not only new ones)" {
  ...
  # claimRef.name만으로 head -1 하면 Retain 고아(Released가 돼도 claimRef를
  # 그대로 들고 있다 — scripts/backup-files-data.sh:99-112 「Bound가 계약」)를 집어 비결정적
