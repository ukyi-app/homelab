#!/usr/bin/env bats
# DR 자산 신선도 posture — 라이브 전용 검증 3종을 한 자리에서 판정한다(owner-local `just verify-posture`).
# 2026-09-03 실측: 세 검증이 전부 owner 수동 호출뿐이라 21일간 (1) committed cert가 라이브
# active cert와 불일치(secret-cert-check STALE → seal-batch가 `just seal-*` 전부 차단) (2) sealing key 백업이
...
#    예: SEALED_KEY_BACKUP_DIR=<git 밖>/sealed-secrets LOCAL_ASSET_BACKUP_DIR=<git 밖>/local-asset just verify-posture
