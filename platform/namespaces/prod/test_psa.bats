...
@test "kustomize build renders all five owned namespaces" {
  run bash -c 'kustomize build platform/namespaces/prod'
  [ "$status" -eq 0 ]
  [ "$(echo "$output" | grep -c '^kind: Namespace')" -eq 5 ]
}

@test "every owned namespace carries a PSA enforce label (no unregulated namespace)" {
  run bash -c 'kustomize build platform/namespaces/prod'
  [ "$status" -eq 0 ]
  # 5개 ns × enforce 라벨 = 정확히 5건
  [ "$(echo "$output" | grep -c 'pod-security.kubernetes.io/enforce:')" -eq 5 ]
}
...
@test "every owned namespace warns at restricted (progressive hardening signal)" {
  run bash -c 'kustomize build platform/namespaces/prod'
  [ "$status" -eq 0 ]
  [ "$(echo "$output" | grep -c 'pod-security.kubernetes.io/warn: restricted')" -eq 5 ]
}
