...
# 그래서 .ci-exclude(gate엔 age 키 없음) — owner-local `just verify-ksops`가 실행(age 키 있으면; cnpg KSOPS bats 선례).
...
@test "manifests are kubeconform-valid (strict)" {
  ...
  #    ksops 부재는 형제 두 레인이 red로 드러내는 편이 정직하다(skip 금지 — `just verify-ksops`가
