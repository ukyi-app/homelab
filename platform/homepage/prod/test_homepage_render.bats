...
@test "configMapGenerator names are bound to the Deployment volume references (assembled nameReference rewrite, F4)" {
  ...
  # ⚠️ 이 정규식 자체가 alertmanager와 같은 클래스의 자동 rollout 의존 가드를
  ...
  #    걸리므로 이 @test와 아래 "generated ConfigMaps enumerate…"가 함께 red다(
  #    실측 — 3가지 철자 전부 확인: 최상위·homepage 단독·homepage-assets 단독).
