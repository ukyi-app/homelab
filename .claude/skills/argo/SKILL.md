...
## 전제
- 라이브 접근: `eval "$(just kubeconfig)"` (= `KUBECONFIG=$PWD/infra/k3s-bootstrap/kubeconfig`).
- 변이(sync/terminate)는 **실행 전 명령을 사용자에게 제시하고 확인**받는다(bypassPermissions 환경이라 프롬프트가 없다).
- 시크릿/`*.enc.yaml` 평문은 출력하지 않는다.

## 1차 진입점 — just 타겟
```bash
just argo-status              # 전 Application: SYNC / HEALTH / 멈춘 OPERATION phase
just APP=<name> argo-sync     # retry 소진 후 명시 sync 트리거
just APP=<name> argo-terminate # 멈춘 operation 종료(phase=Terminating)
just [APP=<name>] argo-wait   # Healthy 될 때까지 대기(미지정=전체)
just COMP=<comp> render       # KSOPS 풀 렌더로 매니페스트 확인(복호 읽기, 라이브 무영향)
```
...
- **실패 리소스가 재시도 안 됨** → ArgoCD는 retry 소진 후 자동 재시도하지 않는다. `just APP=<x> argo-sync`로 명시 sync. 멈춘 op는 `just APP=<x> argo-terminate`.
...
just 타겟이 안 들으면(예: status subresource 형태 차이) AGENTS.md 검증 형태:
```bash
