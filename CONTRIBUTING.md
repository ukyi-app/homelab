...
- **인용부호·구분자 마스킹 함수 재사용** — `mask_semi`/`mask_pipe`류 인용부호·구분자 마스킹 함수를
  ...
  `$'…'`)와 래퍼-대-콘텐츠 경계(래퍼 자체를 마스킹하는지, 언랩한 본문만 마스킹하는지)를 주석에
  명시하고, 아직 그 마스킹을 받지 않은 형제 경로(같은 파일의 다른 판정 루프 포함)를 전수 점검한다
  — 부분 수정이 다음 라운드의 새 finding이 된 재발 사례가 이 파일(`check-bats-style.sh`)에서
  다섯 번째로 반복됐다(mask_semi·mask_pipe·abs_target 언랩·exec_target·bbseg의 do/then/else).
