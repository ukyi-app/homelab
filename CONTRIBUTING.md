...
- **종료코드 규약(가드·도구 공통)** — `tools/lib/cli.ts` 주석이 SSOT: 0=성공 · 1=검증/게이트 실패 ·
  ...
  선언된 예외 1건: `check-credential-expiry`는 붕괴를 2로 낸다 — 소비자 워크플로가 rc 1을
  "만료 임박" 알림 의미로 선점하고 rc≥2만 hard-fail하기 때문(콜사이트 주석·게이트 bats가 근거·값
  보유). 이 목록에 없는 exit 2 붕괴는 1 수렴 대상이다.
