...
# 회전 = `terraform apply -replace=random_password.tunnel_secret` → `just seed-secrets` → PR.
...
# 회전 = `terraform apply -replace=tailscale_oauth_client.k8s_operator` → `just seed-secrets`
