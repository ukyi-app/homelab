...
# sealing-key-dr-gate.sh:155-168 로직 재사용. 라이브 fetch 불가(오프라인)면 검증 스킵(에러 아님).
set -euo pipefail
...
command -v kubeseal >/dev/null 2>&1 || { echo "⚠️ secret-cert-check: kubeseal 없음 — 검증 스킵(라이브 cert 확인 불가)"; exit 0; }

live="$(kubeseal --controller-namespace "$NS" --fetch-cert 2>/dev/null || true)"
if [ -z "$live" ]; then
  echo "⚠️ secret-cert-check: 라이브 cert fetch 실패(KUBECONFIG/클러스터 접근 확인) — 검증 못 함(봉인 전 수동 확인 권장)"
  exit 0
fi
...
if [ -z "$fp_live" ]; then echo "⚠️ secret-cert-check: 라이브 cert 파싱 실패 — 검증 못 함"; exit 0; fi
