...
#    그 주석 줄에서 먼저 끊겨 두 `@bats` 줄을 통째로 놓친다(실측: 그 스코프로 좁히면 sops-roundtrip·
#    sops-guard 두 항목이 **거짓 MISS**로 뒤집혀, 지금 초록인 회계가 새 red를 낸다 — 즉 안전한
#    좁히기가 아니라 회귀다). 주석·빈 줄을 건너뛰고 다음 실제 타깃 선언에서만 멈추는 스코프 +
#    `$(VAR)` 정의 줄 병합까지 다 갖추면 12줄 예산을 확실히 넘는다(형제: tools/check-ci-parity.ts가
#    gate 스텝 본문을 재는 방식과 동급 복잡도). 이번에도 범위 밖으로 남긴다 — 다음 라운드가 이
#    구현을 낸다면 `verify:` 타깃(주석 낀 레시피)을 반드시 회귀 픽스처로 넣을 것.
