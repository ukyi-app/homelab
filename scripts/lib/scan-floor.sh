...
# 사실(권위 경로 회계)과 **그 호출이 가드의 실제 도메인에 닿았다는 사실**은 다른데,
...
#    실측한 적이 있다: 주석은 "11종/27종", 실제는 13종/31종, CONTRIBUTING엔 또 다른 수치.
#    현재값이 필요하면 세어라:
...
#    아래 take_floors(구 env·--min-* 어휘는 전 가드에서 제거됐다).
...
# (TS guardMain의 takeFloors/floorOf 동형. 구 콜사이트별 env·--min-* 어휘를
