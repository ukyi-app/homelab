...
# ⚠️ 여기에 **건수를 적지 않는다.** 예전엔 "11종/27종(셸 8 + TS 3)"이라고 적혀 있었는데 실측은
#    13종/31종(셸 8 + TS 5)이었다 — 아무도 대조하지 않는 손 관리 주장은 반드시 드리프트한다
#    (같은 수치가 CONTRIBUTING·PROGRESS에도 서로 다르게 박혀 있었다). 현재값이 필요하면 세어라:
