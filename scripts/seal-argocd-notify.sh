...
# ⚠️ chatId는 봉인하지 않는다 — notifications-engine은 subscription recipient/템플릿 body에 $secret 확장을
#    service.* 밖에서는 하지 않는다(pkg/api/config.go). chatId는 webhook 템플릿 body에 리터럴 numeric으로 둔다
