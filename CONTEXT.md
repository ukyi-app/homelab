...
주석 표기 · heredoc 상태 기계 · 표면 종류(sh/bats/just/ts/yaml). awk는 규칙을 적힌 **순서로**
...
### 수렴 클래스 (convergence class)

**수렴 자산 (converged asset)**:
라이브가 git에서 갈리면 GitOps 컨트롤러가 라이브를 git으로 되돌리는 tracked 값. git이 곧 SSOT이고
드리프트는 곧 결함이다. 선언이 없는 tracked 값은 이 클래스로 읽는다.
_Avoid_: 관리 대상, 동기화 대상

**시드 (seed)**:
첫 부팅이나 부트스트랩 시점에 한 번만 적용되고 그 뒤로는 수렴되지 않는 tracked 값. 시드를 고쳐도
라이브는 바뀌지 않으며, SSOT는 자산마다 선언한다(라이브·git·시드 전용 — ADR-0007).
_Avoid_: 초기값(재구축에서 다시 쓰이므로 "초기"가 아니다), 기본값, 템플릿

**시드 드리프트 (seed drift)**:
시드와 그 자산의 라이브 값이 갈린 상태. 수렴 자산의 드리프트와 달리 결함이 아니라 **재구축 시
되돌아갈 거리**이며, 신호는 경고이지 red가 아니다.
_Avoid_: OutOfSync(ArgoCD 수렴 자산의 상태이지 시드의 상태가 아니다), 불일치

### 운영 진단 산출물

**운영 수정 초안 (operations change draft)**:
장애 진단에 근거해 이 홈랩의 배포·운영 설정 변경을 제안하는 검토용 패치.
초안 작성 완료는 운영 적용이나 배포 승인을 포함하지 않는다.
_Avoid_: 자동 복구, 배포 완료(초안의 상태와 실행 결과를 혼동한다)
