...
- **`ensure-bump-pr.ts`** — bump PR **멱등 실행기**(조회 → 결정 → 변이를 한 seam에). `bump-poll.yaml`이
  ...
  로컬 커밋을 얹은 뒤 이 도구를 부르면, **원격 변이(push·PR)는 전부 이 도구만** 한다. 관측 사실 =
  ...
  **UNKNOWN**) → `skip`(**push도 create도 없음**) / 신뢰 PR `DIRTY` → `rebuild`(`--force-with-lease=<ref>:<headRefOid>`
  ...
  fail-closed(조용한 중복 create 금지). `--auto-merge`는 생성 후 `scripts/auto-merge-or-fail.sh` 무장.
  ⚠️ lease는 반드시 `<ref>:<기대 OID>` — bare lease는 원격 추적 참조가 없어 stale 거부된다.
  테스트는 `git`/`gh` **PATH stub**으로 argv를 기록해 순서·부작용·플래그를 단언(`tools/tests/test_ensure-bump-pr.bats`),
  호출부 계약은 `tests/gates/test_bump-poll-callsite.bats`.
