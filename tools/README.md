...
- **`check-alert-rules.ts`** — vmalert 룰 expr의 eval-time 안티패턴 정적 lint(`-dryRun`은 파싱만 해서 못 잡는
  ...
  churn 오탐(PR #327) / **모드 C**=push 메트릭(주기 > vmalert instant 룩백 300s)을 rollup 없이 맨 참조하거나
  윈도 < 주기 → 시리즈 구멍 → `for:` pending 리셋 → **영구 무발화**(죽은 알림 PR #339·#341). 모드 C는 in-code
  레지스트리 `PUSH_METRICS`(메트릭→생산자·주기; 주기는 생산자 CronJob cron에서 파생, 불일치=FAIL)와
  **완전성 가드**(`api/v1/import` 호출부가 레지스트리에 없으면 FAIL — 새 push exporter 등록 누락 차단)를 함께
  강제한다. 하한 `W ≥ 주기`만 검사한다(누락 내성 2×·상한 `W < for:`는 e2e preflight 소관 — 헤더 주석 참조).
  면제는 `policy/alert-instance-stability-allowlist.txt`(사유 주석 필수). **`make verify`**·gate가 호출. `--repo-root` 지원.
