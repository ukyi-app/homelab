...
- **`ensure-bump-pr.ts`** — bump PR **멱등 게이트**(읽기 전용 판정). 브랜치를 결정적으로
  (`bump-poll/<app>-<tag>` — RUN_ID 없음) 잡고, PR 생성 직전 `gh pr list --head <branch> --state open
  --json number,isCrossRepository,mergeStateStatus,author` 출력을 `--prs <file|->`로 받아
  `create`/`skip`/`rebuild`를 낸다. **신뢰 경계**: 공개 레포라 포크(cross-repo) PR·타인 PR은 신뢰하지
  않는다(억제 악용 방지) — 동일-레포 + writer App 작성자만 "진행 중"으로 친다. 그 PR이 `DIRTY`면
  `rebuild`(브랜치 재구축 → force-push)로 교착을 푼다. 깨진/빈/스키마 위반 입력은 fail-closed(조용한
  중복 create 금지). 테스트는 `--fixtures <dir>`(`<app>.prs.json`).
