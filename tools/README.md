...
- **`check-alert-rules.ts`** — vmalert 룰 expr의 instance-라벨 불안정 안티패턴 정적 lint(재부팅 IP churn 오탐,
  PR #327). 모드 A=상태-파생 카운터(`policy/alert-instance-stability-denylist.txt`) 위 rollup이 instance를
  안 벗김 / 모드 B=산술 `on()`·`ignoring()` 조인 피연산자가 집계 미포함 raw 셀렉터(422). 면제는
  `policy/alert-instance-stability-allowlist.txt`(사유 주석 필수). **`make verify`**·gate가 호출. `--repo-root` 지원.
