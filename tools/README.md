...
- **`ensure-bump-pr.ts`** — bump PR **멱등 실행기**(조회 → 결정 → 변이를 한 seam에). `bump-poll.yaml`이
  ...
  로컬 커밋을 얹은 뒤 이 도구를 부르면, **원격 변이(push·PR·auto-merge)는 전부 이 도구만** 한다. 관측 사실 =
  `gh pr list --head <branch> --state open --json number,isCrossRepository,mergeStateStatus,author,headRefOid,autoMergeRequest`
  + `git ls-remote --heads origin <branch>`. 판정: 신뢰 PR 없음+브랜치 없음 → `create`(push → `gh pr create`) /
  신뢰 PR 없음+**고아 브랜치** → `adopt`(원격 OID lease force-push → PR) / 신뢰 PR(CLEAN·BEHIND·BLOCKED·
  **UNKNOWN**) → `skip`(**push도 create도 없음**) / 신뢰 PR `DIRTY` → `rebuild`(`--force-with-lease=<ref>:<headRefOid>`
  force-push, PR 재사용). **신뢰 경계**: 공개 레포라 포크(cross-repo)·타인 PR은 신뢰하지 않는다(억제 악용
  방지) — 동일-레포 + writer App 작성자만 "진행 중"으로 친다. 조회 실패·깨진 JSON·스키마 위반은
  fail-closed(조용한 중복 create 금지).
  **레인(`--action`, 필수·기본값 없음)** = 플래너의 `.action`을 **그대로** 넘긴다: `bump`(autoDeploy:true)만
  `scripts/auto-merge-or-fail.sh`로 auto-merge를 무장하고, `propose-pr`은 **절대** 무장하지 않는다(사람 머지
  = 배포 승인). 무장을 켜는 **별도 플래그는 없다** — 있으면 호출부가 두 레인 모두에 넘기는 것만으로 승인
  게이트가 우회된다.
  **무장은 판정과 직교하는 desired state다**: lane=bump이고 신뢰 PR에 `autoMergeRequest`가 없으면 그 run의
  **판정이 무엇이든**(skip이든 rebuild든) 재무장하고, create/adopt는 PR 생성 직후 무장한다. 이미 무장돼
  있으면 손대지 않는다(멱등 — force-push는 무장을 지우지 않는다). 무장 실패가 영구화되면 autoDeploy 배포가
  조용히 정지한다(pr-sweeper는 **이미 무장된** PR만 다룬다). ⚠️ 재무장을 skip 경로에만 매달면
  **DIRTY + 미무장**(라이브에서 겹치는 조합)에서 rebuild만 하고 무장 갭이 남는다.
