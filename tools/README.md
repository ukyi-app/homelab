...
- **`ensure-bump-pr.ts`** — bump PR **멱등 실행기**(조회 → 결정 → 변이를 한 seam에). `bump-poll.yaml`이
  ...
  **한 페이지씩** 소비(`--paginate --slurp` 금지 — 전 페이지를 한 `spawnSync` 캡처에 담으면 버퍼 초과로 gh가
  ...
  브랜치명으로 오염시켜 폴링·회수를 포크 수만큼 태울 수 있었다(R-40). `ref`가 null이면 "우리 브랜치 부재 = 우리 것
  PR 0건"(고아/create 경로 — 조회 실패가 아니다, ls-remote 신호와 정합). 마지막 페이지가 `hasNextPage:true`면
  fail-closed(완전성 증명). **검색 API 금지** — 결과적 일관성이라 직전 주기가 만든 PR이 **거짓 부재**가 된다.
