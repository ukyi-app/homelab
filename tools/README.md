...
- **`db-url.ts`** — 모드 2(실데이터 디버깅): 클러스터 DB에 tailscale로 **읽기 전용** 직결.
  **`bun run db:url --name <db> [--host <ts-ip>]`**. prod의 `db-<name>-ro-conn`에서 `<name>_ro` 롤
  자격(GRANT SELECT only)을 꺼내 host만 tailscale IP로 치환해 `.env.local`에 기록(값 stdout 비노출).
  파괴 수단 없음. 허용 밖 옵션 거부. `--dry-run`은 계획만.
- **`cache-url.ts`** — db-url의 캐시 대칭. **`bun run cache:url --name <cache> [--host <ts-ip>]`**.
  prod의 `cache-<name>-ro-conn`(+@read 전용 ACL 유저)에서 자격을 꺼내 `.env.local`의
  `<NAME>_REDIS_URL`에 기록. 파괴 수단 없음.
- **`env-example.mts`** — `.app-config.yml`(env+secrets+db+redis)에서 `.env.example` 생성.
  **`bun run env:example [--config <f>] [--out <f>]`**. 값은 비움/플레이스홀더(로컬 패리티용).
