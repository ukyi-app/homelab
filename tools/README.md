...
- **`ensure-bump-pr.ts`** — bump PR **멱등 실행기**(조회 → 결정 → 변이를 한 seam에). `bump-poll.yaml`이
  ...
  로컬 커밋을 얹은 뒤 이 도구를 부르면, **원격 변이(push·PR·무장/해제·close)는 전부 이 도구만** 한다.
  **조회 = 상한 없는 완전 열거**: `gh api graphql`의 `repository.pullRequests(headRefName, states:OPEN, first:100)`
  connection을 **한 페이지씩** 소비(`--paginate --slurp` 금지 — 전 페이지를 한 `spawnSync` 캡처에 담으면
  기본 1 MiB 버퍼에서 gh가 살해돼 **포크 포화 = 배포 정지 무기**가 한 층 아래에서 되살아난다) +
  `git ls-remote --heads origin <branch>`. 마지막 페이지가 `hasNextPage:true`면 fail-closed(완전성 증명).
  **검색 API 금지** — 결과적 일관성이라 직전 주기가 만든 PR이 **거짓 부재**가 된다.
  ...
  **절대 무장하지 않고 낡은 무장은 해제**한다(사람 머지 = 배포 승인). 무장을 켜는 **별도 플래그는 없다** —
  ...
  assignee·사람 코멘트·`hold` 라벨·draft·reopen — **잘렸거나 관측 불가면 "흔적 있음"**)은 close도 force-push도 막는다.
  **`--reconcile-only`** = **해제 스윕 전용** 패스(push·create·무장·close 0). 회수는 보안 속성이라 플래너의
  ...
  **더 오래된 형제는 전부 회수**한다(순서 불명 = 전부 회수 — 과잉 회수는 다음 주기가 재무장하지만 과소 회수는
  ...
  비-0 종료(한 앱의 실패가 다른 앱을 굶기지 않는다). superseded 형제 **close**는 증거가 전부 갖춰졌을 때만 하고
  (캡 `CLOSE_MAX`·킬 스위치 `BUMP_PR_CLOSE=off`) **브랜치는 지우지 않는다**.
