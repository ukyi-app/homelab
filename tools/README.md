...
- **`check-alert-rules.ts`** — vmalert 룰 expr의 eval-time 안티패턴 정적 lint(`-dryRun`은 파싱만 해서 못 잡는
  ...
  - **완전성 가드(메트릭 단위)**: 생산자 표면(`platform·scripts·infra·tools·apps·ops·.github`)에서
    `api/v1/import` 호출부를 찾아 **push되는 메트릭 이름을 추출** — 미등록 생산자/미등록 메트릭/추출 실패는
    전부 FAIL(기존 exporter에 메트릭만 추가하는 우회 경로 차단). 역방향(레지스트리 메트릭을 더는 push 안 함)도 FAIL.
