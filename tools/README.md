...
| 파일 | 검증 대상 | 누가 읽나 |
|---|---|---|
| `app-config-schema.json` | **외부 앱 레포**의 `.app-config.yml` 자기선언 (v2 계약). `kind`/`resources` 필수, `secrets`(SealedSecret) 등. 연결(DB/Redis)은 앱 SealedSecret(DATABASE_URL/REDIS_URL), 마이그레이션은 앱 self-migrate — 평문 env 필드 없음. | `create-app.ts`(SSOT), `seal-secret.mts`·`env-example.mts`(앱 레포 측) |
...
- **`db-url.ts`** — 모드 2(실데이터 디버깅): 클러스터 DB에 tailscale 직결 URL을 기록.
  **`bun run db:url --name <db> --host <ts-host> [--rw|--admin]`**. 모드(상호배타): 기본=RO
  ...
  → `.env.admin.local`**(F2 채널 분리 — superuser가 앱 구동/봉인에 안 섞이게). host는 pg-rw-tailscale LB.
...
- **`env-example.mts`** — `.app-config.yml`(secrets)에서 `.env.example` 생성.
  **`bun run env:example [--config <f>] [--out <f>]`**. 값은 비움/플레이스홀더(로컬 패리티용). 연결(DB/Redis)
