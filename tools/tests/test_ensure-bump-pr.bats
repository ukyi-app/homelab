  ...
  api:graphql)
    ...
    case "$q" in
      ...
      *isDraft*)
        ...
        # 본 질의와 달리 mergeStateStatus가 없다(형제는 판정 대상이 아니라 회수 대상이다).
...
# createdAt은 reconcile uniqueNewest의 **순서 근거**다(T_old와 T 사이엔 git 순서가 없다).
# ★ timelineItems = `timelineItems(itemTypes:[REOPENED_EVENT], last:1){ totalCount }`의 응답(H-3) —
#   보고용 humanTouch를 채운다(형제 disarm 판정은 이 값을 보지 않는다).
sib_node() {
...
# ── W35~W44: superseded 형제 — **해제 스윕(넓게·약한 증거·레인 무관·중단 불가)** ─────────────────
...
# 그 피해를 없애는 행동은 **무장 해제 하나**다(넓은 대상: 동일-레포 + writer Bot + 같은 base · 레인 무관 ·
# 소유권/사람흔적 무관 · 중단 불가). 자동 close·브랜치 삭제는 이 도구의 계약이 아니다(파괴는 사람/owner 몫).

# 무장된 완전한 형제 하나를 심는다(회수 대상 — 동일-레포·writer Bot·무장).
...
# bats test_tags=regression
@test "W34: a superseded sibling PR with complete evidence is disarmed while our new PR is created and armed (the sweep never pollutes the main decision)" {
  # 우리 PR은 이번 run에 **새로 만든다**(create) → 열려 있던 형제는 superseded다.
  write_prs '[]'
  setup_closable_sibling
  run_ensure_lane bump
  ...
  run arm_calls_num 999
  [ "$output" -eq 1 ]
  # ③ 형제는 보고에 disarmed로 남는다(무장 해제만 — close는 이 도구의 계약이 아니다).
  echo "$JSON" | jq -e '[.superseded[] | select(.disarmed)] | length == 1' > /dev/null
}

@test "W35: a FORK PR on a sibling branch name is never disarmed (forks are excluded from the sweep)" {
...
@test "W36: a HUMAN-authored PR on a sibling branch name is never disarmed (only writer Bot PRs are swept)" {
  ...
  [ "$status" -eq 0 ]
  run disarm_calls 348
  [ "$output" -eq 0 ] || { echo "사람이 연 PR의 auto-merge를 건드렸다"; dump_calls; false; }
}

# bats test_tags=regression
@test "W37: a sibling whose head commit is NOT ours is still DISARMED (revocation needs no ownership proof)" {
  # ★ 인가 회수는 **안전 방향**이라 소유권 증명을 요구하지 않는다. head가 우리 것임을 증명하지 못해도
  # 그 무장은 회수한다(R-25의 피해는 해제만으로 0이 된다). 소유권 증명을 회수의 조건으로 걸면 낡은 인가가
  # 살아남는다(= R-25 재발).
  write_prs '[]'
  local sb; sb="$(SIB_BRANCH_OF)"
  add_sibling "$sb" "$SIB_OID" "$(sib_node 348 "$SIB_OID" "2026-07-13T06:34:00Z" "$(amr_armed)")"
  ...
  [ "$output" -eq 1 ] || {
    ...
    echo "  해제는 소유권 증명 없이 충분하다(안전 방향)."
    dump_calls; false
  }
}

# bats test_tags=regression
@test "W38: any human trace on a sibling never blocks its DISARM (revocation is unconditional on a superseded PR)" {
  # ★ superseded armed PR은 사람이 만졌든 아니든 **머지될 자격이 없다**(옛 이미지). 사람의 흔적은 그 PR을
  # force-push/파괴로부터 지키지만, auto-merge 무장(=머지 인가)의 회수는 안전 방향이라 **무조건** 한다.
  # 여섯 신호 각각에서 해제가 그대로 일어나야 한다(사람 흔적으로 회수를 막으면 낡은 인가가 산다 = R-25 재발).
  local sb; sb="$(SIB_BRANCH_OF)"
  for f in '.reviews.totalCount=1' '.comments.nodes=[{"author":{"__typename":"User"}}]' \
    ...
    run disarm_calls 348
    [ "$output" -eq 1 ] || { echo "필터 '$f': 사람 흔적 때문에 해제를 건너뛰었다(회수는 무조건이어야 한다)"; dump_calls; false; }
  done
}

# bats test_tags=regression
@test "W39: a sibling NEWER than our PR is still disarmed (the sweep disarms every armed sibling regardless of age)" {
  # 해제 스윕은 이 앱의 **모든** 무장 형제를 회수한다(나이를 보지 않는다 — 나이는 옛 close 순서 근거였을 뿐).
  # 우리 PR보다 나중에 만들어진 형제도 그대로 회수된다: superseded 여부와 무관하게 중복 무장은 위험하다.
  ...
  [ "$output" -eq 1 ] || { echo "더 새로운 형제 PR #421의 무장을 회수하지 않았다"; dump_calls; false; }
}
...
# bats test_tags=regression
@test "W41: every armed superseded sibling is disarmed, however many there are (revocation has no cap)" {
  # 무장된 형제가 4건이든 40건이든 **전부** 회수한다 — 인가 회수는 안전 방향이라 블라스트 캡이 없다.
  write_prs '[]'
  local i=0
  while [ "$i" -lt 4 ]; do
  ...
  disarms="$(count_calls gh pr merge --disable-auto)"
  [ "$disarms" -eq 4 ] || {
    echo "stale authorization survives: 무장 형제 4건 중 일부의 해제를 건너뛰었다(해제 ${disarms}회, 기대 4회)"
    dump_calls; false
  }
}
...
@test "W42: a blind sibling sweep never blocks the main decision, but it can never be reported as success either (an unobservable subject is a revocation failure)" {
  ...
  #      **무장된 좀비 PR**이 있었을 수 있는데, 예전 코드는 warn만 하고 **exit 0**으로 끝냈다
  #      (종료 코드엔 영향이 0이었다 — 종료 코드는 revocationFailures가 정한다) → **회수 대상을 보지도 못한
  #      채 run이 초록이고 telegram도 울리지 않는다**.
...
# bats test_tags=regression
@test "W43: a failed gh pr create still leaves the sibling disarmed (revocation is the safe direction, it precedes the abort)" {
  # 주 판정이 fail-closed(create 출력 파싱 불가)로 죽어도, 형제 해제(①-c 안전 방향)는 이미 끝나 있다.
  write_prs '[]'
  ...
  run disarm_calls 348
  [ "$output" -eq 1 ] || { echo "fail-closed 이전에 형제 무장을 회수하지 않았다"; dump_calls; false; }
}

# bats test_tags=regression
@test "W44: the propose-pr lane also disarms superseded siblings (revocation is lane-independent)" {
  # 형제 해제는 레인을 읽지 않는다 — 승인 레인이든 bump 레인이든 superseded PR은 머지 자격이 없다.
  # 그리고 승인 레인이므로 우리 새 PR은 무장하지 않는다(arm 0).
  write_prs '[]'
  setup_closable_sibling
  run_ensure_lane propose-pr
  [ "$status" -eq 0 ]
...
@test "W45: an orphan sibling ref (no open PR) is observed but never disarmed (nothing to revoke)" {
  # 고아 ref(열린 PR 0)는 회수 대상이 없다 → 보고엔 number:null로 남지만 무장 해제는 일어나지 않는다.
  write_prs '[]'
  local sb; sb="$(SIB_BRANCH_OF)"
  ...
  [ "$status" -eq 0 ]
  merges="$(merge_calls)"
  [ "$merges" -eq 1 ]   # 새로 만든 우리 PR의 무장뿐(고아 형제는 회수 대상이 아니다)
  echo "$JSON" | jq -e '.superseded[0].number == null' > /dev/null
}

# bats test_tags=regression
@test "W46: our own branch is never a sibling of itself (self is never swept or disarmed)" {
  ...
  merges="$(merge_calls)"
  [ "$merges" -eq 0 ] || {
    echo "self-disarm: 실행기가 **자기 PR**(#430, 이미 무장·우리 것)을 superseded로 회수했다"
    dump_calls; false
  }
}
...
# 그리고 이 모드의 변이는 **해제 하나뿐**이다: push·create·무장은 어떤 경로로도 일어나지 않는다.
...
@test "W67: --reconcile-only revokes a bump-lane app's superseded sibling with NO candidate, NO planner, and NO caller-supplied app (the sole revoker must not starve on a noop cycle)" {
  ...
  # 이 모드의 변이는 해제 하나뿐이다(push·create·무장 0).
  pushes="$(count_calls git push)"
  [ "$pushes" -eq 0 ]
  creates="$(count_calls gh pr create)"
  [ "$creates" -eq 0 ]
...
@test "W68: when the createdAt order cannot be established, EVERY arming of that bump app is revoked (at least one of them is certainly superseded)" {
  ...
  # ⚠️ 회수(인가 박탈)는 **애매하면 한다** — 안전 방향이다. 과잉 회수는 다음 주기가 재무장으로 되돌리지만
  #   (R-10), 과소 회수는 무승인 머지다.
  write_bindings '{"autoDeploy": true}'
  local t2="sha-8888888$(printf '%033d' 0)"
  add_sibling "$(SIB_BRANCH_OF)" "$SIB_OID" "$(sib_node 348 "$SIB_OID" "2026-07-13T06:34:00Z" "$(amr_armed)")"
...
# required 체크가 처음부터 다시 돈다. 그래서 사람의 흔적이 있으면 rebuild(force-push)를 하지 않는다.
#
...
#    DIRTY/BEHIND 수렴(rebuild)이 통째로 멈춘다.
...
@test "W63: a hold label beyond the first page of labels blocks the rebuild too (the escape hatch cannot depend on page size)" {
  # `hold`는 사람이 "이 PR은 자동으로 건드리지 마라"고 말하는 **명시적 탈출구**다(force-push를 막는다).
...
# 그 해제가 실패하면 → 무장된 좀비 PR이 남는데 **run은 초록이고 telegram도 울리지 않는다**.
...
@test "W65: a failed sibling disarm lets the main mutation through but still turns the run RED (revocation failure is a security fact)" {
  # 형제는 superseded된 무장 PR이다 → 그 낡은 인가를 거둘 수단은 **해제 하나뿐**이고, 그 해제가 실패한다.
  # 즉 "무장된 채 남는" 그 상태다(그리고 아무도 그 사실을 모른 채 run이 초록으로 끝나면 안 된다).
  ...
  [ "$RC" -ne 0 ] || {
    ...
    echo "  그 PR은 **열린 채 무장된 채** 남는다. 그런데 run은 초록이고 telegram도 울리지 않는다."
    echo "  회수 실패는 **보안 사실**이다 — 조용히 지나갈 수 없다."
    echo "$JSON"; echo "$stderr"; dump_calls; false
  }
...
@test "the harness kills every branch-deleting / ownership-destroying argv (the ref is never taken, the head is never merge-committed)" {
  # ★ 이 도구는 ref를 삭제하지도, head를 머지 커밋으로 만들지도 않는다(**ref 삭제는 되돌아가지 않고**,
  # 머지 커밋은 소유권 증명을 영구 파괴한다). 그 파괴 형태를 stub이 exit 3으로 죽여 계약 밖임을 못박는다.
