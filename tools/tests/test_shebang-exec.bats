#!/usr/bin/env bats
# 셰뱅/exec 비트 정책 가드:
#   - tools/*.mjs는 항상 `node tools/x.mjs`로 호출(exec 비트 없음) → 셰뱅은 dead marker라 금지.
#   - scripts/*.sh는 전부 exec 비트(직접실행/소스 무관 — 일관 정책).
# bash 3.2: 단언은 [ ]만, @test 이름은 영어(한글 인코딩 깨짐 함정).
setup() { ROOT="$(cd "$BATS_TEST_DIRNAME/../.." && pwd)"; cd "$ROOT" || exit 1; }

@test "no tools/*.mjs carries a shebang (always invoked via node — dead marker removed)" {
  bad=""
  for f in $(git ls-files 'tools/*.mjs'); do
