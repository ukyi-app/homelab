...
// owner-local(admin kubeconfig) — DB가 'usable'한지의 단일 권위. **프로덕션 호출자는 없다**(2026-09-09
// 정정 — activate-app도 _create-database.yaml도 이 도구를 부르지 않는다): owner가 온보딩/재프로비저닝
// 때 손으로 대조하는 freshness 증거다. 생산자는 platform/cnpg/prod/ensure-role-password.sh(PostSync 훅).
