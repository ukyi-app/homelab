...
import { APP_NAME_RE, RESOURCE_NAME_RE, EXT_RE } from "./lib/identity.ts";
...
function validateSpec(action: string, specStr: string): void {
  ...
  if (!RESOURCE_NAME_RE.test(String(spec.name ?? ""))) die("spec.name 형식 불량 (소문자 kebab, trailing hyphen 금지, ≤30)");
