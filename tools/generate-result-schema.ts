...
const DEFINITIONS = `    "doctorResult": {
    ...
    "urlResult": {
      "description": "db url/cache url 결과(MCP) — 로컬 .env 파일에 접속 URL을 기록하는 도구의 계획/수행 보고. 평문 자격은 담지 않는다(비출력 계약): 계획(mode/secretRef/envKey/envFile)은 --dry-run으로 캡처하고 실제 기록은 wrote 불리언으로만 표기한다. dryRun=계획만(wrote=false). failure는 error를 담는다.",
      "type": "object",
      "additionalProperties": false,
