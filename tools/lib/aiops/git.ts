...
export class GitSnapshot {
  ...
  git(args: string[], maxBuffer = 8 * 1024 * 1024): string {
    ...
    const result = spawnSync("git", ["-c", "core.hooksPath=/dev/null", "-c", "core.fsmonitor=false", "-C", this.repository, ...args], {
  ...
  blob(path: string, limit = 256 * 1024): Buffer | null {
    ...
    const result = spawnSync("git", ["-C", this.repository, "cat-file", "blob", entry.blob], { timeout: Math.min(10_000, Math.max(1, this.deadline - Date.now())), maxBuffer: limit + 1,
      env: { PATH: process.env.PATH, LANG: "C", GIT_CONFIG_NOSYSTEM: "1", GIT_CONFIG_GLOBAL: "/dev/null", GIT_TERMINAL_PROMPT: "0" } });
...
export function applyCandidate(baseline: GitSnapshot, patch: string): { snapshot: GitSnapshot; dispose: () => void } {
  ...
  const git = (args: string[]) => {
    const result = spawnSync("git", ["-c", "core.hooksPath=/dev/null", "-c", "core.fsmonitor=false", "-C", repository, ...args], {
