...
//     (dispatch-only 폴백은 CLI 암묵 cwd 전용 — owner 결정 2026-09-07).
...
//    지적되어 이연됐다. 그래서 후속 처방은 실측된 드리프트 지점(archetype enum — 확장 시
...
// 그만큼 블로킹되므로, MCP는 아래 짧은 deadline을 **명시해** 그 대기를 바운드한다.
// 반환되는 pending은 두 갈래이고 재개 경로가 서로 다르다:
...
// ⚠️ `onProgress`(변이 엔진의 진행 이벤트 싱크)는 **의도적으로 없다** — 이 서버의 stdout은
...
// (자식 프로세스 이중 실행·계획 키 화이트리스트(옛 땜질)는
...
// (플래그 광고 금지 — 드리프트 방지): 절대 경로, 기준점 없음, 그 자리에 무엇이 생기는가.
...
// 권하는 드리프트가 된다(desc가 `--wait=배포 수렴까지`를 광고했는데 wait는 스키마에 없다).
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        // 확장 시 init 엔진은 수용하는데 MCP만 -32602로 거부하는 입력 표면 드리프트가 난다.
...
export function handleRequest(req: Json): Json | null {
  ...
  if (method === "tools/call") {
    ...
    // 클라이언트 광고일 뿐 신뢰 경계가 아니다. required만 검사하면 null/숫자/빈
    // 문자열 경로가 str()에서 undefined로 접혀 cwd 폴백으로 서버 디렉토리에 변이·자격 기록이 나가고,
    // optional만 검사 밖이면 dryRun:'true'(문자열)가 bool()에서 false로 접혀 실제 자격
