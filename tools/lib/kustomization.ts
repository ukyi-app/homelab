...
export function addResource(kustomizationYaml: string, entry: string): string {
  ...
  // 시퀀스가 없으면(키 부재·null·`""`/`0`/`false` 같은 falsy 스칼라) 새 노드를 만들어 등록하고,
  // 있으면 기존 노드에 추가한다. 두 경로 모두 **같은 노드**에 flow=false를 걸어 block으로 정규화한다 —
  // 그 줄이 없으면 purge cleanup이 남긴 `resources: []`(빈 flow)에서 yaml이 flow를 유지해
  // `resources: [a, b]`로 되살아난다(렌더는 같아도 표기 drift·diff 노이즈). provision-db.ts 자체 헬퍼와 같은 규약.
  // ⚠️ 아래 두 줄의 술어는 반드시 같아야 한다 — `??`(nullish)와 `if (seq)`(truthy)를 섞으면
  //    falsy-비-nullish 스칼라에서 seqNode가 원시값이 되어 flow 대입이 TypeError를 던진다.
  // ⚠️ doc.set("resources", [entry])의 평문 배열은 YAMLSeq가 아니라 JS Array로 트리에 들어가
  //    flow 대입이 조용한 no-op이 된다 — createNode로 노드를 실체화해 그 자리를 막는다.
  // ⚠️ flow→block 전환은 `resources:` 줄에 붙어 있던 꼬리주석을 마지막 항목 뒤 주석 줄로 옮긴다(소실은 아님).
